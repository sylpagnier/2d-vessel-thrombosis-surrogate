...
Oracle validation (the sidecar builder is retired): with a COMSOL ``spf.sr`` sidecar present,
``CLOT_PHI_PHYSICS_GAMMA_MODE=comsol_sr`` +
``CLOT_PHI_PHYSICS_COMSOL_SR_ANCHOR=comsol007`` reproduces GT ``spf.mu`` exactly.

(The trigger-ladder notes this section came from are internal and not published.)
