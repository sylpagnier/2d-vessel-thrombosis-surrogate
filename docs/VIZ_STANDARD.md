...
(`clot_ml_0`). The v1/v3/v4 builders above were removed; only the `clot_ml_0` chain and the off-wall pair ship.
...
6. **State plainly which curves are diagnostic vs shipped.** The lumen arm has no time
   ...
   letting a diagnostic quietly read as a shipped result.
...
8. **If the model has NO held-out set left, say so louder, don't fake one.** `clot_gnn_v3`
   ...
   better" vessels for the panel a substitute for actually holding data out. The FIT/DEV split
   was abandoned because a fixed cut couldn't put the one available aneurysm on both sides of it.
