...
otherwise looking at `comsol001/007/010/013/014/031/042/043`.

---

> ## AMENDMENT, 2026-08-22 — VIZ_HALF IS RELEASED INTO TRAINING
>
> **`comsol001`, `comsol010`, `comsol014`, `comsol042` are no longer held out.** They
...
> models"*) is **superseded for these four**. `comsol042` is a DEV vessel, so it joins
> DEV-train.
>
> **FINAL_HALF is unchanged**: `comsol007`, `comsol013`, `comsol031`, `comsol043` stay
...
> `comsol001` cut placement (band occupancy z = +4.44, cut-gap +0.239) and `comsol042`
> ranking (AUC z = −12.95). Once they are trained on, **those two diagnoses can no longer be
> confirmed out-of-sample.** The offsetting reason to do it: `comsol001`'s 13.5% band
...
> (`comsol017/022/023/026/027/030/033/034`, `maxMat = 0`, empty GT, T = 201) were admitted
...
Every development phase in this project held one 8-vessel SEALED set —
...
|---|---|---|---|
| **VIZ_HALF** | `comsol001`, `comsol010`, `comsol014`, `comsol042` | 042 = stenosis (priority); rest baseline | **open, permanently, for visualization only** |
| **FINAL_HALF** | `comsol007`, `comsol013`, `comsol031`, `comsol043` | 043 = aneurysm (priority); rest baseline | **closed — the project's one remaining true final read** |
...
3. **FINAL_HALF is untouched.** Same rule SEALED always had: never opened, never
   ...
   the phase-9 / phase-10 working notes for `comsol001`'s mask-growth smoke test
   predates this split and should not be read as precedent for touching FINAL_HALF now
   that `comsol001` itself is VIZ_HALF and fair game.
...
`src/core_physics/wall_cohort_splits.py` (`SEALED`) is the canonical, enforced version of this
split — import it in any future viz rather than re-deriving the vessel lists by hand.
