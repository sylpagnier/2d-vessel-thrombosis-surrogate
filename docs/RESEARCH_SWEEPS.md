...
| Id | Axis |
|----|------|
| `01_stenosis_strength` | Stenosis strength = width reduction at the centre, 0-80% in 2.5% steps, 1.25% midpoints at the four visible transitions (38 points), trained taper |
| `02_aneurysm_strength` | Aneurysm strength = width increase at the centre, 0-200% in 10% steps, 2.5% steps across 40-100%, 0.625% across the 45-47.5% onset (42 points), trained taper |
...
**Pathology strength** (`stenosis_occlusion`, `aneurysm_widening`) is the fractional reduction /
increase of the vessel width at the pathology's centre, relative to the inlet width; it is measured
back from the built geometry (`research_sweep_geometry.centre_width_change`) to within 0.5 percentage
points. (`aneurysm_factor`, a wall-offset multiplier where 1.0 meant three times the width, is gone.)
The trained cohort reaches 77% narrowing and 99% widening; points beyond are extrapolation.

Cached sweep graphs are keyed on `GRAPH_BUILDER_VERSION`, so a change to the graph builder (as on
2026-09-14, docs/DEPLOY_ALIGNMENT.md) never reuses a graph built by older code.
