...
kept out of the published tree (see [PUBLISHING.md](PUBLISHING.md)); the modules named
below are a record of what existed, not files you will find in this repository.
...
|-----|-----------------|--------|
| `mat_growth` | Species pushforward, off-wall growth, biochem GNN trainer | `src/training/biochem_species_scope.py` and `clot_trigger_stack.py` were carried into `src/training/` but have no caller left in the tracked `clot_ml_0` build/train/promote/eval chain (only `src/core_physics/t0_mu_physics.py`'s own unused `predict_clot_phi_at_time`/`rollout_t0_clot_phi` reach them); the rest is out of tree |
