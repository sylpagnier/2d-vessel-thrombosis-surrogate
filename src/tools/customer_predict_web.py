...
<title>Local FEM Solver — Customer vessel forecast</title>
<style>
...
<body>
<header class="topbar"><span class="brand">Local FEM Solver</span><span class="brand-sub">Predict</span><select id="app-mode" class="app-mode-select"><option value="clot">Clot Predictor</option><option value="flow">Flow Simulator</option><option value="retrain">Retrain Model</option></select><span class="topbar-note">clot_ml_0 customer vessel forecast</span><span id="status" class="status" aria-live="polite">Ready — choose a geometry to begin.</span></header>
<main class="shell">
<aside class="rail" id="predict-rail">
  ...
  <section class="section" id="advanced-section"><h2 id="advanced-toggle" class="collapsible" role="button" tabindex="0" aria-expanded="false">Advanced<span class="chev">&#9656;</span></h2><div id="advanced-body" class="hidden"><label><input id="steps-override-enabled" type="checkbox"> Override timeline resolution</label><div id="steps-override-controls" class="hidden"><label for="steps-override">Rollout steps</label><div class="range-row"><input id="steps-override" type="range" min="20" max="160" step="5" value="60"><output id="steps-override-out">60</output></div><p class="hint">More steps resolve faster clot dynamics at the cost of runtime. The default auto-scales with simulated time.</p></div><div id="mesh-resolution-row"><label><input id="mesh-resolution-full" type="checkbox"> Full-resolution final solve</label><p class="hint">Draft meshing (default) runs ~3-4x faster with no measured loss of accuracy or convergence. Turn this on for a final run at the exact mesh density the model was trained on. Parametric vessels only.</p></div></div></section>
...
<aside class="rail hidden" id="retrain-rail">
  <section class="section"><h2>Retrain model</h2><label for="retrain-dir">Dataset directory (absolute path)</label><input id="retrain-dir" class="field" type="text" placeholder="C:\path\to\graphs"><p class="hint">Folder of .pt graphs to retrain the biochem GNN on.</p><button id="run-retrain" class="btn primary" type="button" style="margin-top:16px">Start retrain</button><button id="cancel-retrain" class="btn" type="button" disabled style="margin-top:6px;width:100%">Cancel</button><p id="error-retrain" class="error hidden" role="alert"></p></section>
</aside>
...
<script>
(() => {
const $=id=>document.getElementById(id), state={data:null,mode:'clot',frame:0,job:null,jobKind:'predict',snapshots:[],liveResult:null,liveMode:'clot',activeTab:'live'};
...
function renderPreview(){if(source.value!=='parametric'||state.data)return;const c=$('field-canvas'),[w,h,scale]=resizeCanvas(c),ctx=c.getContext('2d');ctx.setTransform(scale,0,0,scale,0,0);ctx.clearRect(0,0,w,h);ctx.fillStyle=cv('--surface2');ctx.fillRect(0,0,w,h);const width=Number($('width').value),bend=Number($('bend').value)*Math.PI/180,amp=Number($('amp').value),arcBend=amp>0?0:bend,pathology=$('pathology').value,woundOn=$('wound-enabled').checked,woundCenter=Number($('wound-position').value)/100,woundWidth=Number($('wound-width').value)/100,pad=38,mid=h/2,base=Math.max(10,width/0.012*h*0.25),pStrength=Number($('p-strength').value),pLocation=Number($('p-location').value),pSharpness=Number($('p-sharpness').value),actualQ=0.2+pLocation*0.6,stdQ=0.035/Math.max(0.01,pSharpness),pts=[];for(let i=0;i<=96;i++){const q=i/96,x=pad+q*(w-2*pad),center=mid+Math.sin(q*Math.PI)*arcBend*h*.18+(amp/.012)*Math.sin(q*Math.PI*2)*h*.28;let radius=base;if(pathology==='stenosis')radius*=1-(0.85*pStrength)*Math.exp(-0.5*Math.pow((q-actualQ)/stdQ,2));if(pathology==='aneurysm')radius*=1+(2.0*pStrength)*Math.exp(-0.5*Math.pow((q-actualQ)/stdQ,2));
// Strength above ~1.25x the configured max occlusion would otherwise drive this negative --
// the wall crossing straight through the centerline into a self-intersecting shape. The actual
// backend geometry build rejects that (min_lumen_width_fraction in config.py); floor it here too
// so strength beyond the safe range saturates into a near-total occlusion instead of inverting.
radius=Math.max(radius,base*0.075);
pts.push([x,center,radius])}
...
const vx=window.view.x/scale,vy=window.view.y/scale,vs=window.view.s,tx=x=>vx+x*vs,ty=y=>vy+y*vs;
...
for(let i=0;i<pts.length;i++){
  if(Math.abs(i/96-woundCenter)>woundWidth/2)continue;
  top.push([tx(pts[i][0]),ty(pts[i][1]-pts[i][2])]);
  bot.push([tx(pts[i][0]),ty(pts[i][1]+pts[i][2])]);
}
...
ctx.globalAlpha=0.22;ctx.fillStyle=cv('--muted');for(let i=0;i<pts.length;i++){for(let j=-3;j<=3;j++){const yy=pts[i][1]+j*(pts[i][2]/3.5);ctx.beginPath();ctx.arc(tx(pts[i][0]),ty(yy),1.4,0,Math.PI*2);ctx.fill()}}ctx.globalAlpha=0.85;ctx.fillStyle='#000';for(let i=0;i<pts.length;i++){ctx.beginPath();ctx.arc(tx(pts[i][0]),ty(pts[i][1]-pts[i][2]),2.8,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(tx(pts[i][0]),ty(pts[i][1]+pts[i][2]),2.8,0,Math.PI*2);ctx.fill()}ctx.globalAlpha=1;if(woundLineA){ctx.save();ctx.globalAlpha=0.9;ctx.strokeStyle=cv('--muted');ctx.lineWidth=1.6;ctx.setLineDash([5,3]);ctx.beginPath();ctx.moveTo(woundLineA[0][0],woundLineA[0][1]);ctx.lineTo(woundLineA[1][0],woundLineA[1][1]);ctx.stroke();ctx.beginPath();ctx.moveTo(woundLineB[0][0],woundLineB[0][1]);ctx.lineTo(woundLineB[1][0],woundLineB[1][1]);ctx.stroke();ctx.restore()}ctx.fillStyle=cv('--muted');ctx.font='12px '+cv('--sans');ctx.fillText(woundOn?'Parametric preview — mirrored wound site shaded':'Parametric geometry preview — run prediction to simulate clot',18,22);$('field-wrap').classList.remove('hidden');$('velocity-wrap').classList.add('hidden');$('science').classList.add('hidden');$('viewer-title').textContent='Parametric vessel preview';$('viewer-time').textContent='Not simulated';time.disabled=true;$('time-out').textContent='Preview'}
function drawField(c,idx){const d=state.data;if(!d)return;const [w,h,scale]=resizeCanvas(c),ctx=c.getContext('2d');ctx.setTransform(scale,0,0,scale,0,0);ctx.clearRect(0,0,w,h);ctx.fillStyle=cv('--surface2');ctx.fillRect(0,0,w,h);const [x0,x1,y0,y1]=extent(d),pad=28,s=Math.min((w-2*pad)/Math.max(x1-x0,1e-9),(h-2*pad)/Math.max(y1-y0,1e-9)),ox=pad+((w-2*pad)-s*(x1-x0))/2,oy=pad+((h-2*pad)-s*(y1-y0))/2,vx=window.view.x/scale,vy=window.view.y/scale,vs=window.view.s,
...
ctx.globalAlpha=1}
function drawVelocity(c,idx){const d=state.data;if(!d||!d.velocity)return;const [w,h,scale]=resizeCanvas(c),ctx=c.getContext('2d');ctx.setTransform(scale,0,0,scale,0,0);ctx.clearRect(0,0,w,h);ctx.fillStyle=cv('--surface2');ctx.fillRect(0,0,w,h);const [x0,x1,y0,y1]=extent(d),pad=28,s=Math.min((w-2*pad)/Math.max(x1-x0,1e-9),(h-2*pad)/Math.max(y1-y0,1e-9)),ox=pad+((w-2*pad)-s*(x1-x0))/2,oy=pad+((h-2*pad)-s*(y1-y0))/2,px=p=>ox+(p[0]-x0)*s,py=p=>oy+(y1-p[1])*s,v=d.velocity[idx]||d.velocity[0],mx=v.reduce((a,b)=>Math.max(a,b),1e-9),velLo=hexRgb(cv('--surface2')),velHi=hexRgb(cv('--teal'));for(let i=0;i<d.pos.length;i++){const q=v[i]/mx,p=d.pos[i];ctx.fillStyle=lerpRgb(velLo,velHi,q);ctx.fillRect(px(p)-2,py(p)-2,4,4)}ctx.fillStyle=cv('--muted');ctx.font='12px '+cv('--sans');ctx.fillText(idx===0?'Initial velocity':'Final velocity',18,22)}
...
function render(){const d=state.data;if(!d)return;const velocity=appMode.value==='flow';$('field-wrap').classList.toggle('hidden',velocity);$('velocity-wrap').classList.toggle('hidden',!velocity);$('science').classList.toggle('hidden',state.mode!=='scientific');time.disabled=velocity;time.max=Math.max(0,d.times.length-1);time.value=state.frame;const title=velocity?'Flow field':(state.mode==='scientific'?'Clot prediction · Scientific':'Clot prediction');$('viewer-title').textContent=title+geometryLabel(d);if(velocity&&d.velocity){drawVelocity($('vel0'),0);drawVelocity($('vel1'),d.velocity.length-1)}else{drawField($('field-canvas'),state.frame);if(state.mode==='scientific')drawScience()}updateStats()}
...
async function poll(){if(!state.job)return;try{const r=await fetch('/api/job/'+state.job),j=await r.json();const elapsed=j.created_at?` · ${duration(Date.now()/1000-j.created_at)} elapsed`:'';setStatus((j.message||j.status)+((j.status==='running'||j.status==='queued')?elapsed:''),j.status==='error'?'err':j.status==='done'?'ok':'');if(j.status==='running'||j.status==='queued'){setTimeout(poll,700);return}$('cancel-run').disabled=true;if(j.status==='cancelled'){setStatus('Cancelled.','err');run.disabled=false;estimate.disabled=false;return}if(j.status==='error')throw new Error(j.error||'Prediction failed');if(j.result&&j.result.kind==='estimate'){$('estimate-note').textContent=`Estimated ${duration(j.result.estimate_low_s)}–${duration(j.result.estimate_high_s)} (central ${duration(j.result.estimate_mid_s)}): ${j.result.n_nodes.toLocaleString()} nodes, ${j.result.n_steps} rollout steps. ${j.result.note}`;setStatus('Runtime estimate ready.','ok')}else{state.data=j.result;state.liveResult=j.result;state.liveMode=j.result.meta.run_mode||$('run-mode').value;state.mode=state.liveMode;state.frame=0;state.activeTab='live';renderTabs();$('snapshot-btn').disabled=false;const dl=$('download');if(j.result.csv_url){dl.href=j.result.csv_url;dl.classList.remove('hidden')}else dl.classList.add('hidden');render();const actual=Number(j.elapsed_s);setStatus(`Prediction complete in ${duration(actual)}.`, 'ok');saveSnapshot()}run.disabled=false;estimate.disabled=false}catch(e){error.textContent=e.message;error.classList.remove('hidden');run.disabled=false;estimate.disabled=false;$('cancel-run').disabled=true}}
async function cancelJob(){if(!state.job)return;$('cancel-run').disabled=true;setStatus('Cancelling…');try{await fetch('/api/job/'+state.job+'/cancel',{method:'POST'})}catch(e){}}

function snapshotLabel(d){const g=d&&d.meta&&d.meta.geometry;const wid=g&&g.width_m?((g.width_m||0)*1e3).toFixed(1)+' mm':'custom';const hrs=d&&d.times&&d.times.length?((d.times[d.times.length-1]||0)/3600).toFixed(1)+'h':'?';const shape=g&&g.curve_type?g.curve_type:'vessel';return `${shape} ${wid} ${hrs}`}
...
function switchTab(idx){state.activeTab=idx;if(idx==='live'){state.data=state.liveResult;state.mode=state.liveMode||'clot'}else{const snap=state.snapshots[idx];if(!snap)return;state.data=snap.data;state.mode=snap.mode}state.frame=0;renderTabs();if(state.data){render();updateStats()}}
...
function saveSnapshot(){if(!state.liveResult)return;const label=snapshotLabel(state.liveResult);state.snapshots.push({label,data:state.liveResult,mode:state.liveMode||state.mode});switchTab(state.snapshots.length-1)}
$('snapshot-btn').addEventListener('click',saveSnapshot);

appMode.addEventListener('change',()=>{const m=appMode.value;$('retrain-rail').classList.toggle('hidden',m!=='retrain');$('predict-rail').classList.toggle('hidden',m==='retrain');$('main-workspace').classList.toggle('hidden',m==='retrain');$('retrain-workspace').classList.toggle('hidden',m!=='retrain');const woundSection=$('wound-enabled').closest('.section');const isFlow=m==='flow';$('hours-row').classList.toggle('hidden',isFlow);$('advanced-section').classList.toggle('hidden',isFlow);$('run-mode-picker').classList.toggle('hidden',isFlow);$('run-section-title').textContent=isFlow?'Run':'Run mode';if(isFlow){$('run-mode').value='clot';$('wound-enabled').checked=false;syncWoundControls();woundSection.classList.add('hidden')}else{woundSection.classList.remove('hidden')}markGeometryDirty()});

$('run-retrain').addEventListener('click',async()=>{error.classList.add('hidden');$('error-retrain').classList.add('hidden');$('run-retrain').disabled=true;$('cancel-retrain').disabled=true;setStatus('Preparing retrain…');retrainLogs=[];$('retrain-console').textContent='';const p={kind:'retrain',data_dir:$('retrain-dir').value};try{const r=await fetch('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)}),j=await r.json();if(!r.ok)throw new Error(j.error||'Could not start job');state.job=j.job_id;$('cancel-retrain').disabled=false;pollRetrain()}catch(e){$('error-retrain').textContent=e.message;$('error-retrain').classList.remove('hidden');setStatus('Ready','err');$('run-retrain').disabled=false}});
async function pollRetrain(){if(!state.job)return;try{const r=await fetch('/api/job/'+state.job),j=await r.json();const msg=j.message||j.status;setStatus(msg,j.status==='error'?'err':j.status==='done'?'ok':'');$('retrain-time').textContent=j.status==='done'?'Complete':(j.status==='error'?'Failed':(j.status==='cancelled'?'Cancelled':'Running…'));if(msg!==retrainLogs[retrainLogs.length-1]){retrainLogs.push(msg);$('retrain-console').textContent+=msg+'\n';$('retrain-console').scrollTop=$('retrain-console').scrollHeight}if(j.status==='running'||j.status==='queued'){setTimeout(pollRetrain,700);return}$('cancel-retrain').disabled=true;if(j.status==='cancelled'){$('run-retrain').disabled=false;return}if(j.status==='error')throw new Error(j.error||'Retrain failed');$('run-retrain').disabled=false}catch(e){$('error-retrain').textContent=e.message;$('error-retrain').classList.remove('hidden');$('run-retrain').disabled=false;$('cancel-retrain').disabled=true}}
async function cancelRetrainJob(){if(!state.job)return;$('cancel-retrain').disabled=true;setStatus('Cancelling…');try{await fetch('/api/job/'+state.job+'/cancel',{method:'POST'})}catch(e){}}

const fc=$('field-canvas');
function repaint(){if(state.data)render();else if(source.value==='parametric')renderPreview()}
fc.addEventListener('wheel',e=>{e.preventDefault();const r=fc.getBoundingClientRect(),d=window.devicePixelRatio||1;const mx=(e.clientX-r.left)*d,my=(e.clientY-r.top)*d;const old=window.view.s;window.view.s*=e.deltaY<0?1.1:0.9;window.view.s=Math.max(1,Math.min(window.view.s,20));window.view.x=mx-(mx-window.view.x)*(window.view.s/old);window.view.y=my-(my-window.view.y)*(window.view.s/old);repaint()});
fc.addEventListener('mousedown',e=>{window.view.drag=1;window.view.sx=e.clientX;window.view.sy=e.clientY});
...
fc.addEventListener('dblclick',()=>{window.view={s:1,x:0,y:0,drag:0,sx:0,sy:0};repaint()});
$('reset-view').addEventListener('click',()=>{window.view={s:1,x:0,y:0,drag:0,sx:0,sy:0};repaint()});

run.addEventListener('click',()=>startJob('predict'));estimate.addEventListener('click',()=>startJob('estimate'));$('cancel-run').addEventListener('click',cancelJob);$('cancel-retrain').addEventListener('click',cancelRetrainJob);$('run-mode').addEventListener('change',e=>{const next=e.target.value;if(state.data&&(next==='clot_velocity'||next==='scientific')&&!state.data.velocity){error.textContent='Re-run the prediction in this mode to include velocity fields.';error.classList.remove('hidden');e.target.value=state.mode;return}state.mode=next;if(state.data)render()});$('run-mode').addEventListener('change',()=>{if($('run-mode').value==='clot_velocity')time.value=0});
})();
</script></body></html>'''
...
def _parametric_params(req: dict[str, Any]) -> dict[str, Any]:
    ...
    if pathology in ("max_stenosis", "max_aneurysm", "stenosis", "aneurysm"):
        params = apply_customer_max_pathology(
            ...
            cfg,
            pathology,
            strength=float(req.get("pathology_strength", 1.0)),
...
def _request_settings(req: dict[str, Any]) -> tuple[float, float, float, int, str]:
    ...
    if mode not in ("clot", "scientific"):
        mode = "clot"
...
def _execute_request(job_id: str, req: dict[str, Any], progress: Any) -> dict[str, Any]:
    if str(req.get("kind")) == "retrain":
        ...
        return {"kind": "retrain", "message": "Retrain complete!"}
    ...
    # Flow Simulator always wants the velocity field -- it has no clot rollout to show.
    include_velocity = mode in ("clot_velocity", "scientific") or str(req.get("app_mode")) == "flow"
...
class Handler(BaseHTTPRequestHandler):
    server_version = "Local FEM SolverWeb/1.0"
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Local FEM Solver customer browser UI")
