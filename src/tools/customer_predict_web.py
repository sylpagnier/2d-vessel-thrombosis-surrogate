...
<aside class="rail hidden" id="retrain-rail">
  <section class="section"><h2>Retrain model</h2><label for="retrain-dir">Dataset directory (absolute path)</label><input id="retrain-dir" class="field" type="text" placeholder="C:\path\to\graphs"><p class="hint">Folder of .pt graphs (already-built, real-labeled vessels) and/or solved .mph COMSOL files. Each file is checked automatically — already-built graphs are used as-is, and .mph files are converted if a matching mesh is alongside them and COMSOL is available on this machine; anything that can't be used is skipped with a reason in the log below.</p><button id="run-retrain" class="btn primary" type="button" style="margin-top:16px">Start retrain</button><button id="cancel-retrain" class="btn" type="button" disabled style="margin-top:6px;width:100%">Cancel</button><p class="hint">This trains a candidate model and scores it on a held-out split — it does not change the live model. A researcher reviews the results and promotes it separately.</p><p id="error-retrain" class="error hidden" role="alert"></p></section>
</aside>
...
async function refreshInbox(){try{const r=await fetchRetry('/api/geometries');const names=await r.json();inbox.innerHTML=names.map(n=>`<option value="${esc(n)}">${esc(n)}</option>`).join('');if(!names.length)inbox.innerHTML='<option value="">No geometry files</option>';}catch(e){setStatus('Could not list geometries','err')}}
...
function drawColorbar(ctx,w,maxVal,unit){const bw=130,bh=10,bx=w-bw-14,by=14;for(let i=0;i<bw;i++){ctx.fillStyle=jetColor(i/bw);ctx.fillRect(bx+i,by,1,bh)}ctx.strokeStyle=cv('--line');ctx.lineWidth=1;ctx.strokeRect(bx+0.5,by+0.5,bw,bh);ctx.fillStyle=cv('--muted');ctx.font='10px '+cv('--mono');ctx.textAlign='left';ctx.fillText('0',bx,by+bh+12);ctx.textAlign='right';ctx.fillText((maxVal>0?Number(maxVal).toPrecision(3):'0')+' '+unit,bx+bw,by+bh+12);ctx.textAlign='left'}
...
function render(){const d=state.data;if(!d)return;const velocity=state.viewAppMode==='flow';$('field-wrap').classList.toggle('hidden',velocity);$('velocity-wrap').classList.toggle('hidden',!velocity);$('science').classList.toggle('hidden',state.mode!=='scientific');$('timeline-row').classList.toggle('hidden',velocity);time.disabled=velocity;time.max=Math.max(0,d.times.length-1);time.value=state.frame;const title=velocity?'Flow field':(state.mode==='scientific'?'Clot prediction · Scientific':'Clot prediction');$('viewer-title').textContent=title+geometryLabel(d);if(velocity&&d.velocity){drawFlowField($('vel0'),0)}else{drawField($('field-canvas'),state.frame);if(state.mode==='scientific'){drawScience();drawAxial();updateSciStats()}}updateStats()}
...
function kindLabel(appMode,runMode){if(appMode==='flow')return 'Flow';return runMode==='scientific'?'Clot · Scientific':'Clot'}
function snapshotLabel(d,appMode,runMode){const g=d&&d.meta&&d.meta.geometry;const wid=g&&g.width_m?((g.width_m||0)*1e3).toFixed(1)+' mm':'custom';const hrs=d&&d.times&&d.times.length?((d.times[d.times.length-1]||0)/3600).toFixed(1)+'h':'?';const shape=g&&g.curve_type?g.curve_type:'vessel';return `${kindLabel(appMode,runMode)} · ${shape} ${wid} ${hrs}`}
...
function saveSnapshot(){if(!state.liveResult)return;const label=snapshotLabel(state.liveResult,state.liveAppMode,state.liveRunMode);state.snapshots.push({label,data:state.liveResult,mode:state.liveMode||state.mode,appMode:state.liveAppMode||'clot'});switchTab(state.snapshots.length-1)}
...
appMode.addEventListener('change',()=>{const m=appMode.value;document.querySelectorAll('.mode-btn').forEach(b=>{const on=b.dataset.mode===m;b.classList.toggle('active',on);b.setAttribute('aria-selected',String(on))});$('mode-hint').textContent=MODE_HINTS[m]||'';$('retrain-rail').classList.toggle('hidden',m!=='retrain');$('predict-rail').classList.toggle('hidden',m==='retrain');$('main-workspace').classList.toggle('hidden',m==='retrain');$('retrain-workspace').classList.toggle('hidden',m!=='retrain');const woundSection=$('wound-enabled').closest('.section');const isFlow=m==='flow';$('hours-row').classList.toggle('hidden',isFlow);$('advanced-section').classList.toggle('hidden',isFlow);$('run-mode-picker').classList.toggle('hidden',isFlow);$('timeline-row').classList.toggle('hidden',isFlow);$('run-section-title').textContent=isFlow?'Run':'Run mode';if(isFlow){$('run-mode').value='clot';$('wound-enabled').checked=false;syncWoundControls();woundSection.classList.add('hidden')}else{woundSection.classList.remove('hidden')}if(m==='retrain'){setStatus('Ready — choose a dataset directory and start retrain.')}else{markGeometryDirty()}});
