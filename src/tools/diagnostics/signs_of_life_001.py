...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 72, flush=True)
    print(f"SIGNS OF LIFE ΓÇö {args.anchor} (steps={args.steps})", flush=True)
    print("=" * 72, flush=True)
    ...
    # for global tiles use None unless we rebuild ΓÇö OK for signs-of-life.
    glob_flow_full = None
    glob_flow_cols = None
    ...
    for feat_source in ("band", "global"):
        ...
        if tile["n_mask"] <= 0:
            print("[ERR] empty frontier_ge2 mask ΓÇö cannot train", flush=True)
