...
#: Hours are real hours. The apps used to define 8 h as 30000 s (3750 s per "hour") so the
#: default horizon landed exactly on COMSOL's; COMSOL's clock is real seconds, so that silently
#: shortened every reported hour by 4%.
SECONDS_PER_HOUR = 3600.0
#: Longest simulated time in the training cohort: 38 of 48 COMSOL anchors run to 30000 s (the
#: rest stop earlier). Nothing past it has ground truth, so the apps do not offer it --
#: the ODE and transport would keep integrating, but the GNN ensemble and the gradient-boosted
#: temporal heads would be fed values outside their training support.
VALIDATED_HORIZON_S = 30000.0
VALIDATED_HORIZON_H = VALIDATED_HORIZON_S / SECONDS_PER_HOUR
...
def frame_scientific_metrics(
    *,
    ...
    out: dict[str, float] = {
        "t_s": float(t_sec),
        "t_h": float(t_sec) / SECONDS_PER_HOUR,
...
def trajectory_scientific_table(
    ...
    *,
    threshold: float = DEFAULT_CLOT_PHI_THRESHOLD,
    seconds_per_hour: float = SECONDS_PER_HOUR,
) -> list[dict[str, float]]:
    ...
    for i in range(int(traj.n_steps)):
        ...
        row["t_h"] = float(fr["t_sec"]) / float(seconds_per_hour)
        rows.append(row)
