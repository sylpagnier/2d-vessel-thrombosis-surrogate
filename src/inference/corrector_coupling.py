...
length scale (``data.x[:, 0:2]`` are already ND on COMSOL anchor graphs), velocity by ``u_ref``,
...
def corrector_max_delta_mu_si() -> float:
    ...
    COMSOL anchor gelation caps mu_eff near ~4 Pa.s -- beyond the patches the corrector saw -- so an
...
def corrector_local_clusters_enabled() -> bool:
    ...
    dx,dy span. Handing it a COMSOL anchor macro-clot (hundreds of connected nodes) as ONE subgraph is
