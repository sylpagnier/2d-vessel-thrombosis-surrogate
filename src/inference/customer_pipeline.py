    ...
    def run(
        ...
        *,
        ...
        if t_final_s is None:
            t_final_s = float(out.t.reshape(-1)[-1])
        # The model runs on its TRAINING timeline, never a display grid: the ODE integrates on
        # COMSOL's 150 s pack axis and the temporal head is queried at the `n_times` indices it
        # was fitted on.  Anything else skews `ti / T`, `r0` and the step-valued off-wall lag.
        axis = trained_time_axis(float(t_final_s))
        if int(out.y.shape[0]) != len(axis):
            from src.data_gen.lib.customer_geometry_import import synthesize_deploy_timeline

            out = synthesize_deploy_timeline(out, t_final_s=float(t_final_s), n_steps=len(axis))
        out.t = torch.as_tensor(axis, dtype=torch.float32)
        ...
        out = out.cpu()
        grid = trained_query_grid(self._bundle["base"]["base"]["temporal"], len(axis))
        ...
        # Frames are the query times themselves: the model predicts nothing in between, so the
        # slider steps through exactly what `clot_ml_0` produces.
        n_steps = len(grid)
        indices = list(range(n_steps))
        phi_all = {
            k: np.asarray(result["series"][int(ti)], dtype=np.float32)
            for k, ti in enumerate(grid)
        }
        ...
        return CustomerTrajectory(
            t_sec=axis[grid].astype(np.float64),
            ...
            meta={
                ...
                "query_step_indices": grid,
                "ode_steps": len(axis),
                "velocity_mode": "single" if include_velocity else "none",
                # True for an uploaded pack's own wound too, not only the mirrored overlay.
                "wound_enabled": bool(has_wound(out)),
