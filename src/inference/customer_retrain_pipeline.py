...
def kill_process_tree(process: subprocess.Popen) -> None:
    """Stop a subprocess and everything it started (a COMSOL session, data-loader workers).

    ``Popen.terminate`` reaches only the direct child.
    """
    ...
    try:
        if sys.platform == "win32":
            ...
            # The child leads its own session (start_new_session below), so its pgid is its pid.
...
class CustomerRetrainPipeline:
    ...
    def run(self, data_dir: Path, progress_cb: Callable[[str], None], log_cb: Callable[[str], None]) -> bool:
        ...
        cmd = [
            ...
            "--data-dir", str(data_dir), "--parent-pid", str(os.getpid()),
        ]
        log_cb(f"> {' '.join(cmd)}")
