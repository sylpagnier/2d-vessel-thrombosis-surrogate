...
#: Is the ODE's TIMING information available to the temporal head?  `[True]` is the shipped
#: behaviour.  Setting `[False]` holds the ODE onset at a constant -- the explicit `oon/T`
#: input AND the two onset columns inside the cached block (`onset_phys`, `onset_phys_ind`) --
#: which is the only ablation in this project that asks whether the integrated ODE contributes
#: to *when* a node clots, as opposed to *whether* it does.
#:
#: What it deliberately does NOT remove: `log_mat_phys` and the rest of the physics MAGNITUDE
#: channels, and the base GNN's out-of-fold score, which was itself trained with the physics
#: conditioning.  So this bounds the ODE's timing contribution FROM BELOW -- a residual amount
#: of physics reaches the head through the score no matter what.  State that when quoting it.
ODE_TIMING = [True]

#: the cached-block columns that carry ODE onset TIMES (not magnitudes)
_ONSET_COLS = ("onset_phys", "onset_phys_ind")
...
def node_features(V, a, oofs):
    ...
    oon = (v["oon"] / v["T"])[:, None]
    if not ODE_TIMING[0]:
        # held at a constant, exactly as `clot_ml.ablation.apply_arm` does it: a column with
        # no variance carries no information, and zeroing after the head's own scaling would
        # inject an out-of-distribution value instead of a neutral one.
        X = np.array(X, copy=True)
        cols = [str(c) for c in S["cols"]]
        for name in _ONSET_COLS:
            if name in cols:
                X[:, cols.index(name)] = 0.0
        oon = np.zeros_like(oon)
    cols_ = [X, np.log1p(np.maximum(v["r0"], 0))[:, None], oon]
    cols_ += [oofs[arm][a][:, None] for arm in sorted(oofs)]
    return np.concatenate(cols_, axis=1)
