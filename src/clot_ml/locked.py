...
def build_sample(data, bio_cfg=None, phys_cfg=None, *, flow: str = "gt",
    ...
    **after** the 55 base channels and **before** ``phys_mask``.  Use
    ...
    if flow == "split":
        # The two scoring domains do not want the same velocity field, and that is measured:
        # the wall score is flow-limited (COMSOL's own t=0 field beats the FEM solve by +0.036,
        # seven times the seed-noise null) while the off-wall score is not (that same perfect
        # field scores -0.014, inside its own null) and loses in proportion to any perturbation
        # of the field.  So the off-wall channels are built on the untouched FEM solve and
        # everything else on the corrected one.  RGP_DEQ_REPAIR_PLAN.md s18.13-18.15.
        #
        # Done HERE rather than by splicing a cache after the fact because every promotion
        # stage after the base GNN -- the temporal head, the wound model, the v0 rollout --
        # rebuilds its features through this function with a single `--flow`, so a cache-level
        # splice trains the base model on one feature distribution and then serves it another.
        from src.clot_ml.v0 import solve_fem_into_pack
        from src.core_physics.flow_sources import OFFWALL_BLOCK

        # A pack carries ONE `u0_pred`, and both halves of the split need their own field, so
        # the FEM side is solved into a clone rather than read from the slot.  Getting this
        # wrong is silent: without the clone both branches read the corrected field, the
        # splice is a no-op, and the result is the plain `rgp` arm wearing this one's name.
        out = build_sample(data, bio, phys, flow="rgp", variant=variant)
        ref_pack = data.clone()
        if not str(getattr(ref_pack, "graph_stem", "") or ""):
            ref_pack.graph_stem = str(getattr(data, "graph_stem", "") or "")
        solve_fem_into_pack(ref_pack)
        ref = build_sample(ref_pack, bio, phys, flow="fem", variant=variant)
        cols = [str(c) for c in out["cols"]]
        if cols != [str(c) for c in ref["cols"]]:
            raise RuntimeError(
                "the 'rgp' and 'fem' feature layouts differ, so a positional splice would mix "
                "channels; rebuild both through the same features.py")
        take = [c for c in OFFWALL_BLOCK if c in cols]
        for name in take:
            j = cols.index(name)
            out["X"][:, j] = ref["X"][:, j]
        out["split_channels_from_fem"] = np.array(take)
        return out
...
#: COMSOL's export step.  Every anchor pack sits on this axis (a run that stops early ends on
#: a shorter final step), so the ODE clock, ``r0`` and the head's ``ti / T`` were all fitted on
#: it.  A deploy timeline on any other step is a train/deploy skew in those features.
COMSOL_DT_S = 150.0


def trained_time_axis(t_final_s: float) -> np.ndarray:
    """The pack time axis a COMSOL run to ``t_final_s`` would have: 150 s steps, exact end."""
    t_end = float(t_final_s)
    return np.append(np.arange(0.0, t_end, COMSOL_DT_S), t_end)


def trained_query_grid(temporal: dict, n_steps: int) -> list[int]:
    """The step indices the temporal head was fitted on, for an ``n_steps`` timeline.

    ``scripts/eval_strict_temporal.py`` (the promote path) queries ``n_times`` evenly spaced
    indices per vessel.  The grid is part of the model, not a display choice: the off-wall
    lag regressor predicts in WHOLE GRID STEPS, so a lag of 4 means 4/10 of the run on the
    trained grid and 4/199 of it on every index of a 201-step pack.
    """
    n_times = int(temporal["n_times"])
    return sorted({int(round(x)) for x in np.linspace(0, int(n_steps) - 1, n_times)})
