...
#: Confirmed by manual read of every anchor's wall mesh, 2026-09-07 (`Vessel Atlas`
#: carousel), extending the original 039-044 designation.  `comsol039`-`comsol047` are the
#: STRAIGHT vessels -- no bend -- so they are the clean, unconfounded axis: a stenosis or
#: aneurysm here is not entangled with bendiness the way it can be on a bent vessel.
#: **This does not mean every other vessel is a clean baseline.**  Bent vessels can carry
#: real stenotic or aneurysmal geometry that was never hand-designated because bendiness
#: dominates the mesh's visual read -- `comsol048` is slightly stenotic and `comsol012` is
#: practically a stenosis with a lot of bend (and independently, `comsol012`'s measured
#: `narrowing` already sits inside the designated-stenosis range -- see the module docstring
#: and `test_the_measured_stenosis_branch_is_known_not_to_separate`). Neither is added here:
#: this dict is for clear-cut, confirmed reads, not borderline calls, and `baseline` in the
#: tables below should be read as "not hand-flagged," not "confirmed normal."
USER_DESIGNATED = {
    "comsol039": "aneurysm", "comsol040": "aneurysm", "comsol043": "aneurysm",
    "comsol047": "aneurysm",
    "comsol041": "stenosis", "comsol042": "stenosis", "comsol044": "stenosis",
    "comsol045": "stenosis", "comsol046": "stenosis",
}
