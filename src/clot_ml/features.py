...
# RESTORED 2026-09-07: the 2026-09-04 env-registry cleanup hardcoded this to "" on the
# premise that "nothing in the tree ever set it" -- true of the PYTHON tree, but
# `CLOT_ML_ORACLE_BLOCKAGE=1 python scripts/eval_clot_ml_0.py` is the documented reproduce
# command for PUBLICATION_NOTES.md s1 and LOCAL_KINEMATIC_CORRECTOR.md's oracle-gate result
# (a human sets it manually; grep for Python setters cannot see that). Same bug class as the
# Stage-A arm knobs (test_stage_a_arm_knobs.py) -- the cleanup silently turned this into a
# no-op and nothing failed, because "off by default, bit-identical when off" made the
# blockage-on and blockage-off runs identical instead of erroring.
CLOT_ML_ORACLE_BLOCKAGE = os.environ.get("CLOT_ML_ORACLE_BLOCKAGE", "")
...
def build_features(data, bio_cfg, phys_cfg, *, flow: str = "gt") -> dict:
    ...
    def _flow_derived(u, v):
        ux, uy, vx, vy = Dx @ u, Dy @ u, Dx @ v, Dy @ v
        scale = u_ref / d_bar
        sr = shear_rate_2d(ux, uy, vx, vy) * scale
        dsrx = (Dx @ sr) / (d_bar * M_TO_CM)
        dsry = (Dy @ sr) / (d_bar * M_TO_CM)
        # Same amplitude correction `t0_flow_fields` applies, through the same helper -- `sgt`
        # is a physical constant and a surrogate `dsrx` is on the 6-hop scale.  FEM and GT
        # resolve to 1.0: converged fields on COMSOL's own scale need no correction.
        ...
        return (sr, dsrx, dsry, (vx - uy) * scale, (ux + vy) * scale, np.hypot(u, v),
                g_low, g_sep, A_branch, A_branch + g_low)

    sr, dsrx, dsry, vort, div, spd, g_low, g_sep, A_branch, gate = _flow_derived(u, v)
    ...
    # GRAMMAR: "<ratio>[@<every>]".  "1" is the measured gelation ratio re-evaluated at EVERY
    # step; "1@16" re-evaluates only every 16th step and holds the gate fixed in between, which
    # is what re-solving the FEM every 16th step would give you.  The stride rides on this
    # existing variable ON PURPOSE -- `src/tests/test_env_registry.py` freezes the config
    # surface so it can only shrink, and a second env var for one research arm is exactly the
    # sprawl that guard exists to stop.
    _blk = None
    _orc = CLOT_ML_ORACLE_BLOCKAGE.strip()
    _fem = re.fullmatch(r"fem(?:@(\d+))?(?:\+(\d+))?|femfinal", _orc.lower())
    if _fem:
        # NON-LEAKING and a REAL flow coupling: re-solves Navier-Stokes against COMSOL's `mu1`
        # step on the rollout's own committed set (docs/publication/EXPERIMENTS.md E1h).
        # GRAMMAR: "fem[@<every>][+<min_new>]" -- re-solve at most every <every> steps (default
        # 16, the point E1e found reaches the every-step gain) and only once <min_new> nodes
        # (default 1) have committed since the last solve.  "fem@1" re-solves on every new clot,
        # which is exactly every step because an unchanged clot gives an unchanged flow;
        # "fem@1+5" waits for 5 new clot nodes.  "femfinal" never re-gates and solves once, at
        # the end, for the flow features alone.
        from src.core_physics.physics_wall_model import fem_resolve_blockage

        _final = _orc.lower() == "femfinal"
        _blk = fem_resolve_blockage(
            data, bio_cfg, f0, wall, hops=hops, flow=flow, update_gate=not _final,
            every=int(_fem.group(1)) if (not _final and _fem.group(1)) else 16,
            min_new=int(_fem.group(2)) if (not _final and _fem.group(2)) else 1)
    elif _orc not in ("", "0"):
        _head, _, _tail = _orc.partition("@")
        _head = _head.strip() or "1"
        _every = int(_tail) if _tail.strip() else 1
        if _head.lower() == "wake":
            # NON-LEAKING closed loop: the gelation wake reads the rollout's OWN committed set
            # (`mat >= crit`) and never touches ground truth, so unlike `oracle_blockage` it
            # does not write the answer into `mat_phys`.  That distinction is the whole point --
            # see docs/publication/STORY.md 2.1f, where the oracle arm was found to leak the
            # label (one feature recovers it at F1 0.96).  The variable is named ORACLE_ for
            # historical reasons and the env surface is frozen; this arm is not an oracle.
            from src.core_physics.gelation_wake import make_gelation_wake_blockage

            _blk = make_gelation_wake_blockage(data, bio_cfg, f0, wall, every=_every)
        else:
            from src.core_physics.physics_wall_model import (GELATION_SR_RATIO,
                                                             oracle_blockage)

            _ratio = GELATION_SR_RATIO if _head == "1" else float(_head)
            _blk = oracle_blockage(data, bio_cfg, f0, hops=hops, ratio=_ratio,
                                   every=_every, phys_cfg=phys_cfg)
    traj, _ = integrate_mat_trajectory(data, bio_cfg, deposition_gate(data, f0, wall=wall),
                                       da_scale=SHIPPED_DA_SCALE, ap_closure=hook,
                                       blockage=_blk)
    mat_phys = traj[-1]
    if hasattr(_blk, "final_velocity"):
        # The coupled arm's flow features describe the flow under the rollout's FINAL committed
        # clot, not the clot-free t=0 field -- otherwise `u`, `v`, `sr` and every aggregate on
        # them stay bit-identical to the uncoupled arm, which is exactly the gap that made the
        # wake arm a test of the ODE gate rather than of flow coupling (STORY 2.1g).
        _vel = _blk.final_velocity(mat_phys)
        u, v = _vel[:, 0].copy(), _vel[:, 1].copy()
        sr, dsrx, dsry, vort, div, spd, g_low, g_sep, A_branch, gate = _flow_derived(u, v)
    u_n = u * nx_ + v * ny_
    u_t = -u * ny_ + v * nx_
    ...
    return dict(F=F, y=gt.astype(np.float32),
                ...
                # FEM re-solves the coupled arm spent on this vessel (0 when uncoupled): the
                # cost axis of the coupling-frequency sweep
                n_flow_solves=int(getattr(_blk, "state", {}).get("solves", 0)) if _blk else 0)
