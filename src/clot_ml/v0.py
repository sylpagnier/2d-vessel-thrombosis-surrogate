...
def load_v0_bundle(name: str | None = None) -> dict:
    """Load a ``unified_v0`` artifact: base GNN (wound-capable) + v0 knobs."""
    import json
    ...
    return dict(base=base, cfg=cfg, manifest=manifest)
...
def chemistry_mat_trajectory(
    ...
    *,
    ...
    Upwind AP renewal + ``da_scale_auto`` + washout + wound-rate blockage.  ``ap_closure``
    is cleared: the dynamic field already subsumes it.
    """
    ...
    from src.core_physics.physics_wall_model import (
        WASHOUT_LAMBDA, deposition_gate, integrate_mat_trajectory,
        t0_flow_fields,
    )
...
def solve_fem_into_pack(data, *, warm_start: bool = True) -> None:
    ...
    """
    synthetic = bool(getattr(data, "research_synthetic", False))
    out = solve_fem_velocity_nd(data, warm_start=warm_start, return_pressure=synthetic)
    nd, p_nd = out if synthetic else (out, None)
    data.u0_pred = torch.tensor(nd[:, 0], dtype=torch.float32)
    data.v0_pred = torch.tensor(nd[:, 1], dtype=torch.float32)
    if p_nd is not None:
        # `p_nd` (t=0 pressure, read from `y[0, :, 2]`) is one of clot_ml_final's 69 features.
        # Every training pack carries COMSOL's solved t=0 pressure there; a customer vessel has
        # no COMSOL field, so it carried 0 -- a feature the model never saw at that value.
        # Fill it from this same solve, in the pack's own units (p / (rho u_ref^2), gauge 0 at
        # the outlet; matches COMSOL's field to 2% rel-L2 on comsol041). Training packs keep
        # their COMSOL pressure, so evaluation of the locked model is unchanged.
        y = data.y
        data.y = y.clone()
        data.y[:, :, 2] = torch.as_tensor(p_nd, dtype=y.dtype).reshape(1, -1)


#: Iteration cap on the warm attempt of a `require_converged` solve; see its docstring.
WARM_ITERS = 90


def solve_fem_velocity_nd(data, *, warm_start: bool = True,
                          delta_mu_nodal_si: np.ndarray | None = None,
                          require_converged: bool = False, max_iters: int = 300,
                          anderson_m: int = 5, return_pressure: bool = False):
    """The deploy FEM solve, returned as ``[N, 2]`` velocity in ``u_ref`` units.

    The one place the deploy inlet treatment and solver settings live, so the clot-coupled
    re-solve (`physics_wall_model.fem_resolve_blockage`) cannot drift from the t=0 solve it is
    compared against.  ``delta_mu_nodal_si`` is the clot viscosity elevation, see
    `solve_local_t0_flow`.

    ``require_converged`` retries a non-converged solve COLD and raises if that fails too,
    instead of returning the field with a warning.  The coupled arm needs it: seeded from the
    PREVIOUS coupled field, comsol012 fell into a limit cycle (300 iterations, relative step
    stuck at ~2e-3) whose field was 0.77 u_ref off the converged one, while the same problem
    converges from the analytic seed in 45 iterations and from cold in 142.  So the coupled arm
    seeds from the analytic prior, like the t=0 solve, and never from its own last answer.

    The analytic seed can stall too (comsol012, 24 committed nodes: damping pinned at 0.25,
    relative step flat at ~2.5e-3 for 300 iterations, field 0.63 u_ref off; exact refactorisation
    every step stalls identically, so it is the damped Picard map, not the frozen LU).  Converged
    warm solves take <= ~60 iterations, so the warm attempt is capped at `WARM_ITERS` and a
    stall costs ~12 s before the cold retry instead of ~145 s.

    Cold can stall too (comsol012 under `fem@1`, 2026-09-13), so two more rungs follow: a
    shorter Anderson history (m=2) and plain Picard (m=0).  On the stalls measured, both reach
    the cold fixed point to 5e-7 where m=5 did not converge -- Anderson only changes the rate, so
    every rung that converges returns the same field.
    """
    import warnings

    if require_converged:
        ladder = [dict(warm_start=True, max_iters=WARM_ITERS)] if warm_start else []
        ladder += [dict(warm_start=False, max_iters=400),
                   dict(warm_start=warm_start, max_iters=500, anderson_m=2),
                   dict(warm_start=False, max_iters=900, anderson_m=0)]
        for k, rung in enumerate(ladder):
            with warnings.catch_warnings(record=True) as caught:
                warnings.simplefilter("always", RuntimeWarning)
                nd = solve_fem_velocity_nd(data, delta_mu_nodal_si=delta_mu_nodal_si, **rung)
            if not any("did not converge" in str(w.message) for w in caught):
                return nd
            print(f"[i] local FEM: solve rung {k} {rung} did not converge", flush=True)
        raise RuntimeError(f"local FEM: no converged solve for "
                           f"{getattr(data, 'graph_stem', '?')} ({len(ladder)} rungs failed)")
    ...
    if warm_start:
        ...
        try:
            ...
            print(f"[i] local FEM: no warm start ({type(exc).__name__}: {exc}); solving cold",
                  flush=True)
            seed, damping = None, 0.5
    u_dim = solve_local_t0_flow(nas_path, data, phys, max_iters=max_iters, tol=1e-9,
                                u_gt_inlet_nd=u_gt_nd, u_init_nd=seed, damping=damping,
                                delta_mu_nodal_si=delta_mu_nodal_si, anderson_m=anderson_m,
                                verbose=delta_mu_nodal_si is None, return_pressure=return_pressure)
    p_dim = None
    if return_pressure:
        u_dim, p_dim = u_dim
    ...
    u_nd = np.asarray(u_dim, dtype=np.float64) / u_ref
    if not return_pressure:
        return u_nd
    p_ref = float(phys.rho) * u_ref ** 2
    return u_nd, np.asarray(p_dim, dtype=np.float64) / p_ref
