...
class ClotGNN(nn.Module):
    def __init__(self, in_dim: int, edim: int, dim: int = 96, layers: int = 6,
                 drop: float = 0.1, extra_dim: int = 0, phys_base: bool = True):
        super().__init__()
        self.extra_dim = int(extra_dim)
        #: `False` is the `B_nobase` ablation: the regression head predicts
        #: `log1p(Mat/crit)` outright instead of a residual on the physics backbone, so
        #: "physics as a base, not a competitor" becomes a measured claim rather than a
        #: design assertion.  `True` reproduces every shipped model exactly.
        self.phys_base = bool(phys_base)
    ...
    def forward(self, x, ei, ea, w_up, w_dn, mat_phys, extra=None):
        ...
        r = self.head_reg(h).reshape(-1)
        reg = (mat_phys + r) if self.phys_base else r
        return logit, reg


#: columns of `edge_features` that carry the FLOW DIRECTION: cos_s, cos_d, cos_s*spd_s.
#: `B_iso` (src/clot_ml/ablation.py) blanks exactly these and makes the two aggregation
#: weights equal, which is the ablation of the anisotropy claim (PHASE6_RESULTS 3.4).  The
#: geometric edge vector and the speed magnitude stay: isotropic does not mean blind.
FLOW_DIR_EDGE_COLS = (4, 5, 6)


def to_device(S: dict, mu: np.ndarray, sd: np.ndarray, dev: torch.device,
              *, iso: bool = False) -> dict:
    ei = S["edge_index"]
    ...
    if iso:
        # ISOTROPIC ABLATION.  Both halves of the update see the same uniform-weight mean, so
        # the layer keeps its width and its parameter count and loses only the direction --
        # the delta is attributable to anisotropy rather than to capacity.
        ea = ea.copy()
        ea[:, list(FLOW_DIR_EDGE_COLS)] = 0.0
        w_up = np.ones_like(w_up)
        w_dn = np.ones_like(w_dn)
...
def build_graph(S, mu, sd, dev_t, *, need_soft=False, need_fb=False, adv_fb=False,
                iso=False):
    g = to_device(S, mu, sd, dev_t, iso=iso)
...
def rollout(model, g, rounds, adv_fb=False, phys_seed=True):
    """K shared-weight refinement rounds; round 0 occlusion is the physics mask.

    ``phys_seed=False`` is the ``B_noseed`` ablation.  The physics backbone reaches the
    network through TWO doors that no amount of feature zeroing can close -- the additive
    regression base, and this seed -- so an A-ladder arm with the physics columns held
    constant still starts its rollout from the backbone's own occlusion mask.  Starting
    from zero instead makes that door measurable (src/clot_ml/ablation.py).
    """
    if rounds <= 1:
        return model(g["x"], g["ei"], g["ea"], g["w_up"], g["w_dn"], g["mat_phys"])
    p = g["phys"].clone() if phys_seed else torch.zeros_like(g["phys"])
    R = int(rounds)
    for k in range(R):
