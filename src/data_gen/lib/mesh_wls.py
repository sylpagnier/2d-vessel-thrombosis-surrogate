...
def wls_sparse_operators(edge_index: torch.Tensor, num_nodes: int, M_inv: torch.Tensor,
                         V: torch.Tensor, W: torch.Tensor):
    """``(G_x, G_y, Laplacian)`` sparse operators from the WLS weights.

    The operators every COMSOL training pack's ``width_d1`` / ``width_d2`` channels were built
    with (`ComsolAnchorDataExtractor`). They are not accurate derivatives (see `mls_gradient`
    for that), but the deployed model was trained on what they produce, so any graph fed to it
    has to be built with them too.
    """
    row, _col = edge_index
    C = torch.bmm(M_inv[row], (W.unsqueeze(1) * V).unsqueeze(2)).squeeze(2)

    def _sparse(edge_weights):
        diag = torch.zeros(num_nodes, dtype=torch.float32, device=C.device)
        diag.scatter_add_(0, row, -edge_weights)
        diag_idx = torch.arange(num_nodes, device=C.device).repeat(2, 1)
        return torch.sparse_coo_tensor(torch.cat([edge_index, diag_idx], dim=1),
                                       torch.cat([edge_weights, diag]),
                                       size=(num_nodes, num_nodes)).coalesce()

    return _sparse(C[:, 0]), _sparse(C[:, 1]), _sparse(C[:, 2] + C[:, 4])
