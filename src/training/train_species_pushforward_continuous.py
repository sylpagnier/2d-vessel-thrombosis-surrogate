...
Usage::
    ...
    python -m src.training.train_species_pushforward_continuous --anchor comsol007 --epochs 120
"""
...
def _build_anchor_pack(
    ...
    *,
    ...
    pack_cache_dir = (
        root / ".cache" / "packs"
        / f"{kine_stem}_hops{wall_hops}_{feat_tag}{prior_source_cache_tag()}"
    )
    pack_cache_path = pack_cache_dir / f"{anchor.strip()}.pt"

    data = None
    static = None
    if pack_cache_path.exists():
        print(f"[i] Pre-loaded species graph cache for {anchor.strip()}", flush=True)
...
def main() -> int:
    ...
    ap.add_argument("--val-anchor", default="comsol007", help="Holdout anchor for val logging")
    ap.add_argument(
            ...
            if (args.deploy_freq > 0 and ep % args.deploy_freq == 0) or ep == args.epochs:
                ...
                if need_clot:
                    ...
                    if sliding_fracs and len(clf_by_t) > 1:
                        ...
                        # (v2's own comsol043 run: mass 1.41 at t=130 -> 2.75 at t=200 within
