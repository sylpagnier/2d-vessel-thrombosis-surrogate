...
from src.clot_ml.locked import build_sample, trained_query_grid
...
def run_clot_ml_0_arm(
    *,
    ...
    # The temporal head was fitted at `n_times` evenly spaced indices per vessel, and the
    # off-wall lag regressor predicts in WHOLE GRID STEPS -- so the query grid is part of the
    # model, not a resolution knob.  Sweeping every index of a denser axis is a different
    # model (`src.clot_ml.locked.trained_query_grid`, docs/DEPLOY_ALIGNMENT.md).
    times = trained_query_grid(
        bundle["base"]["base"]["temporal"], int(data.y.shape[0])
    )
    ...
    # Frames are the QUERY times, not the first `len(keys)` steps of the axis: the model
    # predicts nothing in between, and mislabelling them collapsed `t_s_final` from 30000 s
    # to 1500 s (and with it every `_auc_h` / `_early_rate_per_h`) once the grid stopped
    # being every index.
    frame_t = (
        t_sec[keys] if t_sec.shape[0] > keys[-1]
        else np.linspace(0.0, float(t_sec[-1]), len(keys))
    )
...
def run_sweep(
    ...
    *,
    ...
    arms = list(cfg["arms"])
    if arm_filter:
        # Comma-separated names.  A filtered run MERGES into the existing summary.json below
        # rather than replacing it, so extending a sweep never drops the arms already run.
        wanted = [s.strip() for s in str(arm_filter).split(",") if s.strip()]
        missing = sorted(set(wanted) - {str(a.get("name")) for a in arms})
        if missing:
            raise ValueError(f"No arm named {missing!r} in sweep {cfg.get('id')}")
        arms = [a for a in arms if str(a.get("name")) in wanted]
    ...
    if arm_filter and summary_path.is_file():
        import json

        prev = json.loads(summary_path.read_text(encoding="utf-8"))
        if prev.get("clot_model") not in (None, clot_model if model == DEFAULT_RESEARCH_MODEL else None):
            raise RuntimeError(
                f"{summary_path} was run with clot model {prev.get('clot_model')!r}, not "
                f"{clot_model!r}; refusing to merge arms from two models into one summary")
        fresh = {r["name"]: r for r in summary_rows}
        order = [str(a.get("name")) for a in cfg["arms"]]
        # rows for arms no longer in the config are dropped, so a removed arm cannot linger
        merged = {r["name"]: r for r in prev.get("arms") or [] if r["name"] in order}
        merged.update(fresh)
        summary_rows = sorted(merged.values(), key=lambda r: order.index(r["name"]))
        failed_arms = [f for f in prev.get("failed_arms") or []
                       if f["name"] not in fresh and f["name"] in order] + failed_arms
    ...
    summary = {
        ...
        "n_arms_requested": len(summary_rows) + len(failed_arms),
