...
DEFAULT_T_FINAL_S = 30000.0  # 8.33 h, the COMSOL-validated horizon
#: Steps on the model's OWN axis (COMSOL's 150 s pack cadence), not a display resolution --
#: the ODE clock and the temporal head were both fitted against it.  201 steps for the 30000 s
#: horizon; a custom `t_final_s` gets its own length from `horizon_n_steps`.
DEFAULT_HORIZON_N_STEPS = len(trained_time_axis(DEFAULT_T_FINAL_S))
# Part of every cached graph's key. Bump when `graph_from_mesh_meta` / `synthesize_deploy_timeline`
# change what they build, so a sweep never reuses a graph built by older code. 2026-09-14: inlet
# and outlet corners kept, inlet-width d_bar, bulk t=0 species, training-recipe node channels
# (docs/DEPLOY_ALIGNMENT.md). 2026-09-18: the timeline is the trained 150 s axis, so a cached
# 120-step graph is a different model's timeline.
GRAPH_BUILDER_VERSION = "2026-09-18-trained-time-axis"


def horizon_n_steps(t_final_s: float) -> int:
    """Length of the trained 150 s time axis for a run to ``t_final_s``."""
    return len(trained_time_axis(float(t_final_s)))
...
def build_research_vessel_params(
    *,
    ...
    aneurysm_widening: float | None = None,
    path_loc_frac: float = 0.5,
    ...
    ``stenosis_occlusion`` / ``aneurysm_widening``: pathology STRENGTH, defined as the fractional
    reduction / increase of the vessel width at the pathology's centre (0.5 = 50% narrower /
    wider). `centre_width_change` measures it back from the built geometry.
    """
    cfg = VesselConfig(phase="kinematics")
    rng = np.random.default_rng(int(seed))
    n = int(cfg.num_ctrl_pts)

    if stenosis_occlusion is not None and aneurysm_widening is not None:
        raise ValueError("Specify only one of stenosis_occlusion or aneurysm_widening")
    ...
    elif aneurysm_widening is not None:
        af = float(max(0.0, aneurysm_widening))
        if af <= 1e-12:
            params["v_type"] = "straight"
            params["offsets"] = _zero_noise(n)
        else:
            mag = af * float(width) / 2.0   # each wall moves out by half the widening
...
def arm_geometry_cache_spec(arm: dict[str, Any], control: dict[str, Any]) -> dict[str, Any]:
    ...
    t_final_s = float(arm.get("t_final_s", control.get("t_final_s", DEFAULT_T_FINAL_S)))
    return {
        ...
        "aneurysm_widening": g.get("aneurysm_widening"),
        ...
        "t_final_s": t_final_s,
        # The trained axis, unless a config pins its own length on purpose.
        "n_steps": int(
            arm.get("n_steps", control.get("n_steps", horizon_n_steps(t_final_s)))
        ),
    }
...
def build_research_graph_from_spec(
    ...
    *,
    ...
    return _build_graph(spec, _params_from_spec(spec), work_dir=work_dir)


def _generator_cfg(spec: dict[str, Any]) -> dict[str, Any]:
    cfg_dict = dict(VesselGenerator(phase="kinematics")._cfg_dict())
    cfg_dict["unit"] = "m"
...
def _params_from_spec(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    aneur = spec.get("aneurysm_widening")
    return build_research_vessel_params(
        ...
        aneurysm_widening=None if aneur is None else float(aneur),
...
def centre_width_change(spec: dict[str, Any]) -> float:
    """Measured fractional width change at the pathology centre (negative = narrower).

    Built from the same generator geometry the mesh is made from, measured as inlet-normalised
    wall-to-wall width at the most-deviating station. This is the axis a strength sweep plots.
    """
    from src.data_gen.lib.customer_geometry_import import _resample_polyline
    from src.data_gen.lib.vessel_geometry import compute_geometry_from_params

    geom = compute_geometry_from_params(_params_from_spec(spec), _generator_cfg(spec))
    top = _resample_polyline(np.asarray(geom.top_coords, dtype=np.float64), 400)
    bot = _resample_polyline(np.asarray(geom.bot_coords, dtype=np.float64), 400)
    w = np.linalg.norm(top - bot, axis=1)
    dev = w / w[0] - 1.0
    return float(dev[int(np.argmax(np.abs(dev)))])


def _build_graph(spec: dict[str, Any], params: dict[str, Any], *, work_dir: Path | None) -> Data:
    cfg_dict = _generator_cfg(spec)
    ...
    try:
        ...
        n_steps = int(spec.get("n_steps", horizon_n_steps(t_final_s)))
        out = synthesize_deploy_timeline(data, t_final_s=t_final_s, n_steps=n_steps)
        if n_steps == horizon_n_steps(t_final_s):
            # 150 s steps with the exact end, as a COMSOL pack has it -- `linspace` only
            # coincides with that when the horizon divides evenly.
            out.t = torch.as_tensor(trained_time_axis(t_final_s), dtype=torch.float32)
        return out
    finally:
