...
def deploy_training_pool() -> list[str]:
    """The deploy packs Stage-A may train on: (FIT | DEV) minus everything it must not see.

    Named here because three committed scripts need it to say which vessels a flow model has
    already seen -- `eval_flow_source_paired`'s holdout panel, `eval_domain_routed`'s, and
    `calibrate_residual_scale`'s choice of tuning set -- and they were importing it from
    `scripts/stage_a/`, which is gitignored exploration.  On a fresh clone that is an
    ImportError in the middle of an evaluation.

    Excluded, and why each: SEALED is the project's one unspent final read; VIZ_RELEASED may be
    shown but never selected on; the stale-extractor and known-bad stems carry anomalous prior
    blocks; and the selection subset is what a checkpoint is CHOSEN on, so training on it would
    make selection meaningless.
    """
    from src.core_physics.wall_cohort_splits import DEV, FIT, SEALED, VIZ_RELEASED
...
def load_deploy_training_packs(*, prior_source: str | None = None, verbose: bool = True):
    ...
    **Disjoint from selection by construction.**  Every stem the run selects on is excluded
    ...
    if verbose:
        # Recomputed from the pool rather than from a local the refactor deleted: what is held
        # out is exactly (FIT | DEV) minus the legal pool, and deriving it that way cannot
        # drift from `deploy_training_pool` the way a second copy of the ban list would.
        from src.core_physics.wall_cohort_splits import DEV, FIT

        held = sorted((set(FIT) | set(DEV)) - set(deploy_training_pool()))
        print(f"[kin] Deploy TRAINING packs: {len(out)} (prior_source={source}); "
              f"held out of training: {', '.join(held)}")
    return out


__all__ = ["deploy_training_pool", "load_deploy_training_packs", "load_selection_packs",
           "selection_pack_dir", "selection_pack_stems", "selection_subset_stems", "use_stems"]
