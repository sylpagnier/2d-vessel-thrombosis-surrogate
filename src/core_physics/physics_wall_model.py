...
def t0_flow_fields(
    ...
    velocity: tuple[np.ndarray, np.ndarray] | None = None, D=None,
) -> T0Fields:
    ...
    ``velocity`` -- explicit ``(u, v)`` in ``u_ref`` units, overriding the pack's slots; the
    clot-coupled FEM re-solve differentiates its field through here so it cannot drift from
    the transcription the t=0 field used.  ``D`` reuses a prebuilt ``(Dx, Dy)``.
    """
    pos = node_positions(data)
    ei = data.edge_index.detach().cpu().numpy()
    u_ref = float(data.u_ref.reshape(-1)[0])           # m/s
    d_bar = float(data.d_bar.reshape(-1)[0])           # m
    Dx, Dy = build_mls_gradient(pos, ei, hops=hops) if D is None else D
    ...
    if velocity is not None:
        u = np.asarray(velocity[0], dtype=np.float64).reshape(-1)
        v = np.asarray(velocity[1], dtype=np.float64).reshape(-1)
        sr = shear_rate_2d(Dx @ u, Dy @ u, Dx @ v, Dy @ v) * (u_ref / d_bar)
    elif flow_source in flow_sources.RECONSTRUCTED:
...
#: Median Carreau viscosity at t=0 [Pa.s] (0.00651 on comsol001, matched to COMSOL's `spf.mu`
#: within 1.5%, see `local_fem_solver.effective_viscosity`).  COMSOL's clot step multiplies it
#: by `mu_ratio_max`, so the elevation a gelled node carries is `(mu_ratio_max - 1) * this`.
MU_BASE_SI: float = 0.0065


def clot_delta_mu_si(bio_cfg) -> float:
    """COMSOL's physical `mu1(Mat)` step as an elevation over the Carreau baseline, ~0.51 Pa.s."""
    return (float(bio_cfg.mu_ratio_max) - 1.0) * MU_BASE_SI


def fem_resolve_blockage(data, bio_cfg, fields, wall: np.ndarray, *, hops: int = 3,
                         every: int = 16, min_new: int = 1, update_gate: bool = True,
                         flow: str = "fem", delta_mu_si: float | None = None):
    """``blockage`` callable that closes the clot->flow loop with an ACTUAL Navier-Stokes re-solve.

    THE EXPERIMENT EVERY EARLIER COUPLING ARM STOOD IN FOR (docs/publication/EXPERIMENTS.md
    E1h).  `oracle_blockage` read GT clot and leaked the label; the gelation wake is clean but
    only rescales `sr` inside the gate and leaves `u`/`v` untouched.  This one:

      1. takes the rollout's OWN committed set, ``mat >= viscosity_mat_crit`` -- no GT;
      2. raises the viscosity there by COMSOL's `mu1` step (:func:`clot_delta_mu_si`) and
         re-solves the deploy FEM (:func:`src.clot_ml.v0.solve_fem_velocity_nd`) to a
         CONVERGED field -- seeded from the analytic prior, never from its own last answer,
         which put comsol012 in a limit cycle 0.77 u_ref off the solution;
      3. re-differentiates through :func:`t0_flow_fields` and re-gates through
         :func:`deposition_gate`, so the wound keeps its ungated law.

    It re-solves at most every ``every`` steps AND only once at least ``min_new`` nodes have
    committed since the last solve.  With ``min_new=1`` that is "whenever the committed set
    changed" -- an unchanged viscosity field gives the same flow, so skipping it is exact, and
    ``every=1, min_new=1`` is therefore identical to re-solving at every step.  Hysteresis as
    in the other arms: a committed node never loses the gate it ignited with.

    ``update_gate=False`` never re-gates during the rollout (the ODE sees the t=0 gate) and
    only ``final_velocity`` solves: coupling in the flow FEATURES alone, one solve per vessel.
    It separates what the coupled field adds as a network input from what it adds by steering
    the deposition dynamics.

    ``blockage.final_velocity(mat)`` returns the flow under the committed set of ``mat``
    (re-solving if it differs from the last one), so the flow FEATURES can be rebuilt from the
    coupled field rather than the t=0 one.  ``state["solves"]`` counts FEM calls.
    """
    from src.clot_ml.v0 import solve_fem_velocity_nd  # noqa: PLC0415

    crit = float(bio_cfg.viscosity_mat_crit)
    dmu = clot_delta_mu_si(bio_cfg) if delta_mu_si is None else float(delta_mu_si)
    wall = np.asarray(wall, dtype=bool)
    D = build_mls_gradient(node_positions(data), data.edge_index.detach().cpu().numpy(),
                           hops=hops)
    if fields.u is None or fields.v is None:
        raise ValueError("fem_resolve_blockage needs the t=0 velocity on `fields` as its seed")
    vel0 = np.stack([np.asarray(fields.u), np.asarray(fields.v)], axis=1).astype(np.float64)
    state: dict = {"gate": None, "last": -10 ** 9, "occ": None, "vel": vel0, "solves": 0}

    def _solve(occ):
        if state["occ"] is not None and np.array_equal(occ, state["occ"]):
            return state["vel"]
        vel = solve_fem_velocity_nd(data, delta_mu_nodal_si=np.where(occ, dmu, 0.0),
                                    require_converged=True)
        state.update(vel=vel, occ=occ.copy(), solves=state["solves"] + 1)
        return vel

    def blockage(mat, gate0, i):
        occ = np.asarray(mat) >= crit
        if not update_gate or not occ.any():
            return gate0
        n_new = int(occ.sum()) if state["occ"] is None else int((occ & ~state["occ"]).sum())
        if state["gate"] is None and n_new < int(min_new):
            return gate0
        if state["gate"] is not None and (i - state["last"] < int(every)
                                          or n_new < int(min_new)):
            return state["gate"]
        vel = _solve(occ)
        f = t0_flow_fields(data, bio_cfg, hops=hops, flow_source=flow,
                           velocity=(vel[:, 0], vel[:, 1]), D=D)
        g = deposition_gate(data, gate=f.gate, wall=wall)
        g = np.where(occ, np.maximum(g, gate0), g)
        state.update(gate=g, last=i)
        return g

    def final_velocity(mat):
        occ = np.asarray(mat) >= crit
        return state["vel"] if not occ.any() else _solve(occ)

    blockage.state = state
    blockage.final_velocity = final_velocity
    return blockage
