...
def solve_local_t0_flow(mesh_path, data, phys_cfg: PhysicsConfig, max_iters=300, tol=1e-8,
                        ...
                        verbose: bool = True, return_pressure: bool = False):
    ...
    if delta_mu_nodal_si is not None:
        ...
        # CLIPPED AT ZERO, and this is what made physical clot viscosities diverge.  A clot is a
        # step, and P2 shape functions over- and undershoot a step: on comsol012 a nodal
        # 0 -> 0.51 Pa.s step interpolates to -0.127 Pa.s at ~830 quadrature points, 36x below
        # the Carreau floor `mu_inf`.  Negative total viscosity makes the viscous form indefinite
        # and the solve blew up (|v|max 605 against 0.42) -- read for two days as "the solver
        # cannot converge above ~0.1 Pa.s".  The nodal values are already non-negative, so the
        # clip only removes interpolation ringing, never clot.
        dmu_qp = np.clip(np.asarray(_sb.interpolate(vec)), 0.0, None)
    ...
    # Pressure is P1: vertex DOFs only. A mid-side node (on the file or registered above) takes the
    # mean of its edge's two vertices, which is exactly the P1 field evaluated there.
    p_vert = x[basis.nodal_dofs[2, :]]
    p_skfem = np.zeros(mesh.p.shape[1])
    p_skfem[:basis.nodal_dofs.shape[1]] = p_vert
    if mesh.p.shape[1] > basis.nodal_dofs.shape[1]:
        p_skfem[basis.nodal_dofs.shape[1]:] = 0.5 * (p_vert[mesh.facets[0]] + p_vert[mesh.facets[1]])
    p_pred = np.zeros(len(pos_target_nd))
    p_pred[skfem_to_target] = p_skfem
    if mesh.p.shape[1] == basis.nodal_dofs.shape[1] and fkeep.any():
        p_facet = 0.5 * (p_vert[mesh.facets[0]] + p_vert[mesh.facets[1]])
        p_pred[fidx[fkeep]] = p_facet[fkeep]
    return u_pred, torch.tensor(p_pred, dtype=torch.float32)
