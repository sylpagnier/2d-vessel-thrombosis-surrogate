"""Release invariants for the C0-tail ``clot_ml_v0`` baseline alias."""
...
def test_clot_ml_v0_binds_the_reviewed_c0_and_wound_artifacts():
    path = LOCKED / "clot_ml_v0" / "manifest.json"
    if not path.exists():
        return
...
def test_clot_ml_v0_records_generalization_and_deploy_gates():
    path = LOCKED / "clot_ml_v0" / "manifest.json"
    if not path.exists():
        return
    alias = json.loads(path.read_text())
