...
def test_mesh_without_triangles_errors(tmp_path: Path) -> None:
    # A sidecar is no longer required (the boundary comes from the outline), so an empty mesh
    # fails on its own content with a message a customer can act on.
    msh = tmp_path / "lonely.msh"
    msh.write_text("$MeshFormat\n2.2 0 8\n$EndMeshFormat\n", encoding="utf-8")
    with pytest.raises(CustomerGeometryError, match="no triangles"):
...
def test_uploaded_pt_cannot_execute_code_on_load(tmp_path):
    """A crafted .pt must be refused by the loader, not unpickled."""
    import pickle
    ...
    torch.save({"x": Payload()}, bad, pickle_protocol=pickle.DEFAULT_PROTOCOL)
    with pytest.raises(Exception):
        load_customer_geometry(bad)
