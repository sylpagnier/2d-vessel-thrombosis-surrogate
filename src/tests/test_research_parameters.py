...
class _FakeTraj:
    ...
    def __init__(self):
        ...
        # 0, 4, 8 h at an explicit 3750 s per hour (the scale is a parameter)
...
def test_research_timeseries_matches_scientific_keys():
    rows = compute_research_timeseries(_FakeTraj(), seconds_per_hour=3750.0)
    assert len(rows) == 3
...
def test_research_parameters_from_trajectory_pack_shape():
    pack = research_parameters_from_trajectory(_FakeTraj(), seconds_per_hour=3750.0)
