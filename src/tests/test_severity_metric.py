...
def test_locked_ensemble_manifest_is_consistent():
    ...
    root = repo / p["path"]
    man_path = root / p["manifest"].split("/")[-1]
    if not man_path.exists():
        # the pointer is tracked; the artifact it names lives in gitignored outputs/
        pytest.skip(f"{p['name']} artifact not present in this checkout")
...
def test_geometry_classifier_reproduces_the_designated_class():
    ...
    if not stats:
        pytest.skip("anchor graph packs not present in this checkout")
