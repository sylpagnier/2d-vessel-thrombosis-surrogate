...
def test_app_wound_toggle_off_keeps_an_uploaded_packs_own_wound():
    """Toggle off adds no wound; it must not erase a GT wound pack's wound into healthy wall."""
    from src.tools.customer_predict_web import _apply_requested_wound

    wounded = apply_customer_mirrored_wound(
        _straight_channel(), enabled=True, position_frac=0.50, width_frac=0.30
    )
    kept = _apply_requested_wound(wounded, {"wound_enabled": False})
    assert torch.equal(kept.mask_wound, wounded.mask_wound)
    assert torch.equal(kept.mask_wall, wounded.mask_wall)
