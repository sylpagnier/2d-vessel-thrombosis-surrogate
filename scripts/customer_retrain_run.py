...
from src.clot_ml.recipe import CUSTOMER_RETRAIN_EPOCHS, recipe
...
def _safe_stem(name: str, taken: Container[str] = ()) -> str:
    """Anchor name for a customer file, distinct from every name in ``taken``.

    Sanitizing is lossy (``a b.pt`` and ``a_b.pt`` both give ``customer_a_b``, as do ``a.pt``
    and ``a.mph``), and a repeated name silently replaced the earlier anchor -- so a clash
    takes a numeric suffix instead.
    """
    import re
    ...
    unique, n = stem, 2
    while unique in taken or unique in CANONICAL_NAMES:
...
def _validate_pt_graph(path: Path) -> tuple[object | None, str]:
    ...
    try:
        data = load_untrusted_graph(path)
...
def discover_and_classify(data_dir: Path, report: RunReport) -> dict[str, object]:
    ...
    for p in pt_files:
        ...
        if data is None:
            ...
            continue
        stem = _safe_stem(p.stem, accepted)
        accepted[stem] = data
...
def main() -> int:
    ...
    ap.add_argument("--parent-pid", type=int, default=None,
                    help="exit when this process does (set by the UI that launched the run)")
    args = ap.parse_args()
    if args.parent_pid is not None:
        exit_when_parent_dies(args.parent_pid)
