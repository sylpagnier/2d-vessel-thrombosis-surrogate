...
def main() -> int:
    ...
    ap.add_argument("--flow", default="gt", choices=["gt", "pred", "fem"],
                    help="t=0 velocity the head is fitted against.  It MUST be the one "
                         "deploy replays -- the committed set, the ODE clock and the "
                         "transport channels are all built from it.  Recorded on the "
                         "artifact so `predict_temporal_v4` cannot be called with another.")
    ap.add_argument("--pool", nargs="*", default=None,
                    help="override the clot-carrying pool (default: eligible_pool())")
    args = ap.parse_args()
    ...
    pool = [a for a in (args.pool or eligible_pool()) if (PACKS / f"{a}.pt").exists()]
    ...
    for a in pool:
        ...
        S = build_sample(d, bio, flow=args.flow, variant="v4")
        cache[a] = S
    ...
    artifact = dict(
        ...
        # The t=0 flow this head was fitted against.  `predict_temporal_v4` warns when it is
        # called with another one, because the committed set, the ODE clock and the transport
        # channels are all functions of that field -- swapping it silently is the mistake that
        # made a `flow="fem"` run score ground truth (docs/DEPLOY_FLOW_PLAN.md).
        flow=str(args.flow))
    ...
    manifest["temporal_readout"] = dict(wall=wall_spec["kind"], off=off_spec["kind"],
                                        ...
                                        wake_ode=bool(args.wake), stall_ode=bool(args.stall),
                                        flow=str(args.flow))
    manifest["flow"] = str(args.flow)
    manifest["temporal_pool"] = list(pool)
