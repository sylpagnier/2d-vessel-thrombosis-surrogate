...
from src.clot_ml.strict_readout import (  # noqa: E402
    N_PARAMS, apply_adapt, readout_plain, readout_resid, vessel_stat,
)
...
from src.clot_ml.evaluate import strict_f1  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--labels", default="flow_branch", choices=["flow_branch", "comsol"],
                    help="'flow_branch' (default) scores a registered mirror-branch vessel against its "
                         "mirrored GT (src/clot_ml/mirror_branch.py). 'comsol' keeps COMSOL's labels "
                         "as given -- ONLY for GT-oracle coupling arms (cpl_closed*, cpl_c16*), whose "
                         "features carry COMSOL's own clot and so follow COMSOL's branch")
    ap.add_argument("--save", default="")
    args = ap.parse_args()

    cfg = DEFAULT if args.metric == "severity" else LEGACY
    mirror = args.labels == "flow_branch"
    ...
    vs = {a: BoundScorer(cache[a]["edge_index"], eval_gt(a, cache[a]["y"], cache[a]["pos"], mirror=mirror),
    ...
    for k, held in sorted(folds.items()):
        ...
        for a in held:
            ...
            pr = (w & pw) | (offd & po)
            gt_a = eval_gt(a, cache[a]["y"], cache[a]["pos"], mirror=mirror)
            rows[a] = dict(wall=vs[a].score(pr & w, w),
                           off=vs[a].score(pr & offd, offd),
                           # node-exact F1 of the SAME prediction, beside the severity score
                           wall_f1=strict_f1(pr, gt_a, w),
                           off_f1=strict_f1(pr, gt_a, offd),
    ...
    if args.save:
        Path(args.save).write_text(json.dumps(
            dict(rows=rows, phys=phys, chosen={str(k): v for k, v in chosen.items()},
                 tags=args.tags, cache=args.cache, metric=args.metric, labels=args.labels),
