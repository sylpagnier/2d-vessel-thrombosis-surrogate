"""Assemble outputs/v5_vs_v6_wound_patient003.json into the standard two-window viz for clot_gnn_v4."""
from __future__ import annotations

import json
from pathlib import Path

DATA_PATH = Path("outputs/v5_vs_v6_wound_patient003.json")
OUT_PATH = Path("outputs/phase10_v4_temporal_report.html")

CLASS_COLOR = {"wound test": "badge-priority", "aneurysm": "badge-priority", "stenosis": "badge-priority",
              "baseline": "badge-baseline", "unknown": "badge-baseline"}
CLASS_TEXT = {"wound test": "WOUND", "aneurysm": "ANEURYSM", "stenosis": "STENOSIS",
             "baseline": "BASELINE", "unknown": "GEOM N/A"}
CLASS_NAME = {
    "wound test": "Wound Test",
    "aneurysm": "Aneurysm", "stenosis": "stenosis",
...
def main() -> None:
    ...
    model_name = payload[order[0]].get("model", "clot_gnn_v4")
    ...
    held_out = [v for v in order if payload[v].get("sealed_viz")]
    ex_viz_half = [v for v in order if payload[v].get("was_viz_half") and not payload[v].get("sealed_viz")]
    in_sample = [v for v in order if not payload[v].get("sealed_viz") and not payload[v].get("was_viz_half")]
...
TEMPLATE = r"""<title>Phase 10 &mdash; __MODEL__ vs Ground Truth</title>
<style>
:root {
