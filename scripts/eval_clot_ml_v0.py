"""Compare ``clot_ml_v0`` against the best past baseline (default: ``clot_gnn_v5w``).
...
``wall`` / ``off`` (true lumen).  SEALED is never in the default list.
    ...
    python scripts/eval_clot_ml_v0.py
    python scripts/eval_clot_ml_v0.py --stems wound_patient003 patient012
    python scripts/eval_clot_ml_v0.py --cohort
"""
...
from src.clot_ml.v0 import load_v0_bundle, predict_clot_ml_v0  # noqa: E402
...
def _score_one(bundle_base, bundle_v0, stem: str, every: int, flow: str) -> dict:
    ...
    v0 = predict_clot_ml_v0(bundle_v0, data, times, flow=flow, sample=S)
...
def main() -> int:
    ...
    ap.add_argument("--v0", default="clot_ml_v0")
    ...
    print()
    print("NON-WOUND (global wall / true-lumen off)")
