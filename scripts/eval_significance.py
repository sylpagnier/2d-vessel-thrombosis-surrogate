...
def nested_rows(cache, tags, family=None):
    ...
    ``family`` pins the readout family instead of choosing it on the selection set.  The
    default ``None`` chooses, which is what every existing caller measured.  Pinning
    ``"plain"`` is what the ABLATION report needs: the ``resid`` family reads the physics
    backbone's own occlusion mask, so under ``auto`` even a geometry-only arm can be scored
    through a physics-derived readout, and the ladder would understate what physics buys.
    """
    ...
    for k, held in sorted(folds.items()):
        ...
        best = None
        cands = FAMILIES.items() if family is None else [(family, FAMILIES[family])]
        for fam, (tune, apply_) in cands:
