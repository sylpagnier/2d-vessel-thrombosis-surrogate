# Shared PowerShell launcher helpers for active 2D Thrombosis Surrogate scripts.
...
# The local Predict launcher runs the SAME browser app both release zips run (run.bat /
# run_unix.sh: `-m src.tools.customer_predict_web`). There used to be a separate Tk desktop UI
# here, so what we tested locally and what customers downloaded silently drifted apart;
# src/tests/test_customer_predict_web.py now fails if the two entry points diverge again.
function Invoke-GoCustomerPredict {
    param([switch] $Cpu)
    Invoke-GoCustomerPredictWeb -Cpu:$Cpu
}
...
function Invoke-GoCustomerPredictWeb {
    ...
    Write-GoBanner -Title "2D Thrombosis Surrogate web predict: http://${BindHost}:${Port}" -InfoLines @(
