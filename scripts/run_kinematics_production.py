"""Stage-A production launcher (foundation -> polish -> clinical -> promote).
...
Usage:
  ...
  python scripts/run_kinematics_production.py clinical --holdout patient007
"""
...
_SUBCOMMANDS = frozenset({"ladder", "foundation", "finetune", "clinical"})
...
def _ladder_from_ns(ns: argparse.Namespace) -> LadderConfig:
    return LadderConfig(
        ...
        clinical=ClinicalFinetuneConfig(
            holdout=str(ns.holdout),
            finetune_epochs=int(getattr(ns, "clinical_finetune_epochs", 25)),
        ),
        skip_foundation=bool(getattr(ns, "skip_foundation", False)),
        skip_synthetic_polish=bool(getattr(ns, "skip_synthetic_polish", False)),
        skip_clinical_anchors=bool(getattr(ns, "skip_clinical_anchors", False)),
        skip_promote=bool(getattr(ns, "skip_promote", False)),
        require_clinical=bool(getattr(ns, "require_clinical", False)),
    )
...
def _build_production_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--skip-clinical-anchors", action="store_true")
    ap.add_argument("--skip-promote", action="store_true")
    ap.add_argument("--require-clinical", action="store_true")
    ap.add_argument("--holdout", type=str, default="patient007")
    ap.add_argument("--no-continuity-focus", action="store_true")
    ap.add_argument("--synthetic-finetune-epochs", type=int, default=40)
    ap.add_argument("--clinical-finetune-epochs", type=int, default=25)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv_in and argv_in[0] in _SUBCOMMANDS:
        ...
        if cmd == "clinical":
            ap = argparse.ArgumentParser(description="Clinical anchor finetune")
            ap.add_argument(
                "--resume",
                type=str,
