...
from src.clot_ml.recipe import PROMOTION_EPOCHS, recipe
from src.utils.paths import anchor_packs_dir, get_project_root

REPO = get_project_root()

from src.clot_ml.ablation import ARCH_ARMS, ARMS, apply_arm, describe_arm  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--arm", default="A4", choices=sorted(ARMS),
                    help="ablation arm (src/clot_ml/ablation.py).  'A4' is the shipped "
                         "configuration and changes nothing: no column is held constant and "
                         "no architecture switch is flipped, so `--arm A4` reproduces the "
                         "run this flag did not exist for, bit for bit.")
    ...
    # --- ABLATION -------------------------------------------------------------------------
    # The architecture switches are applied AFTER argparse so the arm is the single source of
    # truth for what it changes; `A4` and every `A*` arm but `A1_pure` leave all three at the
    # shipped default.  They are recorded in `cfg` and therefore travel into the npz's `cfg`
    # entry and into every saved fold member, so a score file always says which model made it.
    cfg.iso, cfg.phys_base, cfg.phys_seed = False, True, True
    for k, v in ARCH_ARMS.get(args.arm, {}).items():
        setattr(cfg, k, v)
    cfg.arm = args.arm
    ...
    print("[i] %s" % describe_arm(args.arm), flush=True)
    # AFTER `attach_physics`: the `phys_mask` column it appends is part of the partition, and
    # it reads the real `sdf_nd` out of `X` on the way through.
    zeroed = apply_arm(cache, args.arm)
    ...
    store["arm"] = np.array([args.arm])
    store["arm_zeroed"] = np.array(zeroed)
