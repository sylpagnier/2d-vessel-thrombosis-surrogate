...
The promoted ``clot_ml_v0`` weights train on the complete non-sealed pool.  They are useful
...
def build_wound_payload(stems: list[str], *, every: int, flow: str) -> dict:
    ...
    out: dict[str, dict] = {"_meta": dict(
        schema_version=1, model="clot_ml_v0 wound LOVO", mode="wound", flow=flow,
    ...
    for anchor in stems:
        ...
        pred = predict_clot_ml_v0(bundle, data, times, flow=flow)
...
def main() -> None:
    ...
    if args.wound:
        out_path = Path(args.out or "outputs/clot_ml_v0_wound_temporal_data.json")
    ...
    out_path = Path(args.out or "outputs/clot_ml_v0_oof_temporal_data.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
