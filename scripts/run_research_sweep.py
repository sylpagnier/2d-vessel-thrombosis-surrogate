...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--arm", type=str, default="", help="Optional comma-separated arm names; merged into the existing summary.json")
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Deploy-clot artifact to roll forward, overriding the config's `clot_model` and "
             "the `clot_ml_0` default.  This is how a NEW generation's sweeps are produced "
             "without moving the shipped pointer: repointing to draw a figure is an "
             "outward-facing change that should not be a side effect of an experiment.",
    )
    ...
    for path in paths:
        try:
            ...
            if args.clot_model.strip():
                cfg["clot_model"] = args.clot_model.strip()
            pipeline = None
