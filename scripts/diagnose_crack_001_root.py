...
from src.evaluation.deploy_ckpt_recipe import apply_deploy_ckpt_recipe, load_eval_static  # noqa: E402
...
from src.utils.kinematics_inference import resolve_kinematics_checkpoint  # noqa: E402
...
def _static_eval_style(data, device, kine, anchor: str, wall_hops: int = 3) -> dict:
    """Match deploy wall-band static construction."""
    static = load_eval_static(data, device, kine, wall_hops, anchor)
...
def main() -> int:
    ...
    apply_deploy_ckpt_recipe(dict(payload.get("meta") or {}), label="wall", ckpt_path=wall_ckpt)
    ...
    for anc in anchors:
        ...
        st_eval = _static_eval_style(data, device, kine, anc, wall_hops=3)
        print(
