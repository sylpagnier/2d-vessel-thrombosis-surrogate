...
from src.core_physics.flow_provenance import ABSENT, SOLVED, classify, pack_stamp
...
def main() -> int:
    ...
    for stem in stems:
        try:
            ...
            d["stem"] = stem
            # Which weights produced the field this row measured.  `u0_pred` is one slot that
            # every reconstructed source writes, so without this the row cannot be told apart
            # from the same vessel measured under a different arm.
            # `fem` and `gt` do not come from a checkpoint at all -- the FEM field is solved
            # a few lines above and GT is COMSOL's own -- so stamping the pack's residual
            # precache provenance onto those rows would name weights that had no part in the
            # number.  Only a reconstructed arm carries a checkpoint.
            stamp = ({"run": SOLVED, "checkpoint": SOLVED} if a.flow in ("fem", "gt")
                     else pack_stamp(data))
            d.update({f"flow_{k}": v for k, v in stamp.items()})
            rows.append(d)
    ...
    # Wrapped, not bare: the old payload was a naked list with no record of WHICH flow
    # checkpoint was resident, so a later precache could invalidate the file while it still
    # looked current.  `read_diagnostics` accepts both shapes, so older files still load.
    provenance = {
        "mode": classify([r.get("flow_run", ABSENT) for r in rows]),
        "runs": dict(sorted(Counter(r.get("flow_run", ABSENT) for r in rows).items())),
        "n_vessels": len(rows),
    }
    payload = {"flow": a.flow, "provenance": provenance, "vessels": rows}
    Path(a.out).write_text(json.dumps(payload, indent=2), encoding="utf-8")
    print(f"  flow provenance: mode={provenance['mode']}  runs={provenance['runs']}")
