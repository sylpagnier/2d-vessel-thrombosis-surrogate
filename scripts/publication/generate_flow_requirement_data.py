...
def _pairs(gt_rows: list[dict], pred_rows: list[dict]) -> list[dict]:
    ...
    MIRROR-BRANCH VESSELS ARE EXCLUDED, and neither of this panel's two axes is measurable on
    them.  Their steady problem is bistable: our field solves COMSOL's reflection
    (`src/clot_ml/mirror_branch.py`), so (i) every diagnostic here compares the surrogate with
    COMSOL's UNREFLECTED field and measures the reflection, not the surrogate -- rel-L2 0.53 and
    0.67 on comsol045/046 against 0.007-0.057 everywhere else -- and (ii) the `flow=gt` baseline
    runs on COMSOL's branch while `mirror_branch.eval_gt` scores it against mirrored labels, so
    its drop comes out NEGATIVE by construction.  Left in, those two points carry the panel:
    they flip rel-L2's correlation to -0.378 (Spearman +0.07: two outliers, not a trend) and
    halve gate Jaccard's.  Scoring them properly would need a `flow=gt` rerun on the reflected
    COMSOL field; until then they are not evidence either way.
    """
    from src.clot_ml.mirror_branch import mirror_branch_stems  # noqa: PLC0415

    skip = set(mirror_branch_stems())
    gt = {r["stem"]: r for r in gt_rows if "stem" in r}
    out = []
    for r in pred_rows:
        stem = r.get("stem")
        if stem is None or stem not in gt or stem in skip:
            continue
