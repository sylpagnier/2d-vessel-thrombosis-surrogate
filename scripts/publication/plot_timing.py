...
def main() -> int:
    ...
    ax0.set_xticks(x)
    # Anchors were renamed patient00N -> comsol00N (2026-09-04); label by the number alone.
    ax0.set_xticklabels([r["stem"].replace("wound_comsol", "w").replace("comsol", "")
                         for r in rows], rotation=90, fontsize=CONFIG.font_size - 4)
    ax0.set_ylabel("wall-clock (s)")
    ax0.set_title(f"(a) cost per vessel · flow solve {pct_by_stage['FEM t=0']:.0f}% of runtime",
                  loc="left", fontsize=CONFIG.font_size)
    ax0.set_ylim(0, bottom.max() * 1.15)
    ax0.legend(frameon=False, fontsize=CONFIG.font_size - 2, loc="upper left", ncol=3)
    ...
    ax1.set_title(f"(b) {speedup:,.0f}$\\times$ vs COMSOL", loc="left")
    ...
    fig.savefig(out)
    fig.savefig(out.with_suffix(".png"), dpi=CONFIG.dpi)
    plt.close(fig)
