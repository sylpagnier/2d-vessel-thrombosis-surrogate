...
def build_vessel_figure_data(
    ...
    *,
    ...
    # solved first: `features.build_features` reads `data.u0_pred`, and `build_sample` does
    # NOT solve for `flow="fem"` -- it reads the slot.
    #
    # ALWAYS re-solve, never "solve if the slot looks empty".  `u0_pred` is a single slot that
    # every flow source writes, so its PRESENCE says nothing about its PROVENANCE: this guard
    # was `not hasattr(data, "u0_pred")`, and a pack that had been through any precache always
    # has the attribute.  The solve was therefore skipped and `build_sample(flow="fem")` --
    # which reads the slot rather than solving -- silently consumed whatever field was last
    # written there.  Measured on comsol020 (2026-09-06): the resident field differed from the
    # true FEM solve by 4.4e-2 relative, i.e. figures 3/4/6 and Table 4 were built on an
    # RGP-DEQ field and labelled `fem`.
    #
    # The solve is deterministic and warm-started (~1 s/vessel here), so doing it every time
    # costs little and removes the whole class of bug. `eval_strict_temporal.precompute` has
    # always solved unconditionally; this brings the figure path in line with it.
    if oof.flow == "fem":
