...
with the CURRENTLY SHIPPED model (load_default -> whatever `clot_ml_0` resolves to, currently
`clot_ml_final_0`), not the strict-OOF ensemble the rest of this pipeline uses, because the
shipped model is wound-aware and the strict-OOF cohort excludes wound vessels entirely.
