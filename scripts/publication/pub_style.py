...
def annotate_scores(ax, *, wall_score: float | None, off_score: float | None) -> None:
    """
    Add wall / off-wall clot scores as text inside the panel lower-left.

    Deliberately NOT labelled with the metric name -- at 7pt in a panel corner there is no
    room, and repeating it on every panel is noise.  The name belongs in the caption, and is
    recorded per-artifact under the `metric` key of `table4_kfold.json` /
    `operating_point.json` (`publication.utils.metric_identity`), which is where a caption
    writer should read it rather than remembering which of BATC / BATC_0 was configured.
