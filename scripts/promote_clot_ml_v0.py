"""Lock ``clot_ml_v0`` -- one artifact for wounded and non-wounded vessels.
...
On a pack with no wound it returns the base GNN unchanged -- asserted here, not argued.
    ...
    python scripts/promote_clot_ml_v0.py
    python scripts/promote_clot_ml_v0.py --base clot_gnn_v5w --repoint
    python scripts/eval_clot_ml_v0.py          # compare against --baseline (v5w)
"""
...
from src.clot_ml.v0 import (  # noqa: E402
    DEFAULT_NAME, KIND, REPLACE_SCOPES, ClotMlV0Config, load_v0_bundle, predict_clot_ml_v0,
)
...
def _assert_noop_without_wound(bundle: dict, base_name: str) -> list[str]:
    ...
    for stem in NOWOUND_CHECK:
        ...
        b = predict_clot_ml_v0(bundle, data, times, flow="gt")
...
def _assert_wound_is_covered(bundle: dict) -> dict:
    ...
    for stem in WOUND_STEMS:
        ...
        T = int(data.y.shape[0])
        w = wound_mask(data)
        pred = predict_clot_ml_v0(bundle, data, [T - 1], flow="gt")
