...
from src.clot_ml.locked import (  # noqa: E402
    build_sample, load_temporal_v4_wound, predict_temporal_v4_wound, trained_query_grid,
)
...
def _score_one(bundle_base, bundle_v0, stem: str, every: int | None, flow: str) -> dict:
    ...
    # DEFAULT: the grid the temporal head was FITTED on (`n_times` evenly spaced indices, as
    # `eval_strict_temporal.py` queries them).  The off-wall lag regressor predicts in whole
    # grid steps, so `--every` scores a different model -- kept only as an explicit override.
    times = (
        _times(data, every) if every
        else trained_query_grid(bundle_v0["base"]["base"]["temporal"], int(data.y.shape[0]))
    )
    if flow == "fem":
...
def main() -> int:
    ...
    ap.add_argument("--every", type=int, default=0,
                    help="OVERRIDE the trained query grid with every Nth pack frame (1 = every "
                         "frame). Off by default: the head's own grid is part of the model, and "
                         "any other one scores something else.")
    ...
    grid_label = f"every {args.every}" if args.every else "trained query grid"
    print(f"[i] baseline={args.baseline}  v0={args.v0}  n={len(stems)}  grid={grid_label}",
          flush=True)
