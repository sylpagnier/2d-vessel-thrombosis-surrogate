...
RESTORED 2026-09-16 from the dist copy.  It was removed from the tree on 2026-09-07, which left
`crossfit5_vs_shipped.json` and `split_vs_shipped.json` unregenerable.  Two changes: arms take
comma-separated seed tags, and labels are the evaluation labels on the flow branch
(`src/clot_ml/mirror_branch.py`).

    python scripts/eval_flow_source_paired.py --a dc_fem_cfw025,dc_fem_seedB --a-cache v5_fem \\
        --b dc_xf5,dc_xf5_seedB --b-cache v5_xf5 --out outputs/deployclot/crossfit5_vs_shipped.json
"""
...
def main() -> int:
    ...
    ap.add_argument("--a", required=True, help="baseline tag(s), comma-separated seeds")
    ap.add_argument("--a-cache", required=True)
    ap.add_argument("--b", required=True, help="comparison tag(s), comma-separated seeds")
