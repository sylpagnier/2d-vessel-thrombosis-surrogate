...
def main() -> int:
    ...
    parser.add_argument("--solenoidal", action="store_true",
                        help="project the model's residual (pred - prior) onto the discretely "
                             "divergence-free fields before storing it.  The off-wall feature "
                             "block transports material along the flow, and a transport solve "
                             "on a field that is not solenoidal creates and destroys the "
                             "material it is meant to conserve: measured, the residual raises "
                             "|div u|/|u| 8-52x over the FEM prior and the off-wall deploy "
                             "score falls monotonically with it "
                             "(RGP_DEQ_REPAIR_PLAN.md s18.12).")
    ...
    provenance = {
        ...
        "solenoidal": bool(args.solenoidal),
    ...
    for file_path in pt_files:
        ...
        if args.solenoidal:
            # The prior is the hard BC's base point and is already solenoidal to 5e-5, so only
            # the model's own contribution needs projecting -- and projecting only the residual
            # keeps the FEM solve's divergence exactly rather than re-deriving it.
            import numpy as _np

            from src.config import NodeFeat as _NF
            from src.core_physics.mls_gradient import build_mls_gradient as _mls
            from src.core_physics.solenoidal_residual import project_solenoidal as _proj

            prior_uv = data_cuda.x[:, _NF.UV_PRIOR].detach().cpu().numpy().astype(_np.float64)
            uv = _np.stack([u0.detach().cpu().numpy(), v0.detach().cpu().numpy()], axis=1)
            pos_np = _np.asarray(node_positions(data), dtype=_np.float64)
            Dx, Dy = _mls(pos_np, data.edge_index.detach().cpu().numpy(), hops=3)
            proj, pinfo = _proj(uv.astype(_np.float64) - prior_uv, Dx, Dy,
                                wall=_np.asarray(getattr(data, "mask_wall", None)))
            uv = prior_uv + proj
            u0 = torch.tensor(uv[:, 0], dtype=u0.dtype)
            v0 = torch.tensor(uv[:, 1], dtype=v0.dtype)
            print("      solenoidal: |div| %.3e -> %.3e, residual moved %.1f%%"
                  % (pinfo["div_before"], pinfo["div_after"], 100 * pinfo["moved"]), flush=True)
