...
$ZipPath = Join-Path $RepoRoot "dist\2DThrombosisSurrogate-Predict-win64-$Version.zip"
$UnixZip = Join-Path $RepoRoot "dist\2DThrombosisSurrogate-Predict-macos-linux-$Version.zip"
foreach ($z in @($ZipPath, $UnixZip)) {
    if (-not (Test-Path $z)) { throw "Build did not produce the expected zip: $z" }
}
# The zips carry pickled checkpoints that load with code execution, so a copy from anywhere
# but this release must be checkable: publish SHA-256 digests as an asset and in the notes.
$SumsPath = Join-Path $RepoRoot "dist\SHA256SUMS.txt"
$SumLines = foreach ($z in @($ZipPath, $UnixZip)) {
    "$((Get-FileHash -Path $z -Algorithm SHA256).Hash.ToLowerInvariant())  $(Split-Path -Leaf $z)"
}
[System.IO.File]::WriteAllText($SumsPath, (($SumLines -join "`n") + "`n"))
$ChecksumNotes = @"
## Verify your download

Compare the SHA-256 of the zip you downloaded with the matching line below (also attached as ``SHA256SUMS.txt``):

``````
$($SumLines -join "`n")
``````

- Windows (PowerShell): ``Get-FileHash .\<zip> -Algorithm SHA256``
- macOS: ``shasum -a 256 <zip>``
- Linux: ``sha256sum <zip>``
"@
$SizeMb = [math]::Round((Get-Item $ZipPath).Length / 1MB, 1)
$UnixSizeMb = [math]::Round((Get-Item $UnixZip).Length / 1MB, 1)
...
Write-Host "  Assets:   $ZipPath ($SizeMb MB)"
Write-Host "            $UnixZip ($UnixSizeMb MB)"
Write-Host "            $SumsPath"
...
git tag -a $Tag -m "2D Thrombosis Surrogate Predict $Tag"
...
try {
    gh release create $Tag $ZipPath $UnixZip $SumsPath `
        --title "2D Thrombosis Surrogate Predict $Tag" `
        --notes $ChecksumNotes `
        --generate-notes
